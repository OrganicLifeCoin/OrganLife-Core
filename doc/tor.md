...
In a typical situation, this suffices to run behind a Tor proxy:
    ...
    ./organiclifed -proxy=127.0.0.1:9050
...
To see verbose Tor information in the organiclifed debug log, pass `-debug=tor`.
...
methods to be configured: cookie authentication or organiclifed's `-torpassword`
configuration option.

#### Cookie authentication

For cookie authentication, the user running organiclifed must have read access to
the `CookieAuthFile` specified in the Tor configuration. In some cases this is
preconfigured and the creation of an onion service is automatic. Don't forget to
use the `-debug=tor` organiclifed configuration option to enable Tor debug logging.

If a permissions problem is seen in the debug log, e.g. `tor: Authentication
cookie /run/tor/control.authcookie could not be opened (check permissions)`, it
can be resolved by adding both the user running Tor and the user running
organiclifed to the same Tor group and setting permissions appropriately.
...
run organiclifed, run this as root:
...
your organiclifed's P2P listen port (51472 by default), and target addresses and ports
...
In a typical situation, where you're only reachable via Tor, this should suffice:
    ...
    ./organiclifed -proxy=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -listen

(obviously, replace the .onion address with your own). It should be noted that you still
listen on all devices and another node could establish a clearnet connection, when knowing
your address. To mitigate this, additionally bind the address of your Tor proxy:

    ./organiclifed ... -bind=127.0.0.1

If you don't care too much about hiding your node, and want to be reachable on IPv4
as well, use `discover` instead:

    ./organiclifed ... -discover
...
for normal IPv4/IPv6 communication, use:
    ...
    ./organiclifed -onion=127.0.0.1:9050 -externalip=7zvj7a2imdgkdbg4f2dryd5rgtrn7upivr5eeij4cicjh65pooxeshid.onion -discover
