...
BASE_SCRIPTS= [
    ...
    'feature_governance_coinlock.py',
...
def main():
    ...
    enable_organiclifed = config["components"].getboolean("ENABLE_BITCOIND")
    ...
    if not (enable_wallet and enable_utils and enable_organiclifed):
        print("No functional tests to run. Wallet, utils, and organiclifed must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, keep_cache="rewrite"):
    # Warn if organiclifed is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "organiclifed"]) is not None:
            print("%sWARNING!%s There is already a organiclifed process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "BITCOIND" not in os.environ:
        os.environ["BITCOIND"] = build_dir + '/src/organiclifed' + exeext
        os.environ["BITCOINCLI"] = build_dir + '/src/organiclife-cli' + exeext
...
class TestHandler:
    ...
    def __init__(self, num_tests_parallel, tests_dir, tmpdir, test_list=None, flags=None):
        ...
        self.flags = flags
        self.num_running = 0
        # In case there is a graveyard of zombie organiclifeds, we can apply a
...
class RPCCoverage():
    ...
    commands per `organiclife-cli help` (`rpc_interface.txt`).
