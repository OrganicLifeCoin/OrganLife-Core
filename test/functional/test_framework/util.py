...
def rpc_url(datadir, i, rpchost=None, chain="regtest"):
...
def initialize_datadir(dirname, n, chain="regtest"):
...
def get_auth_cookie(datadir, chain="regtest"):
    user = None
    password = None
...
def get_collateral_vout(json_tx):
    # Legacy tests that manually fund collateral expect an exact 100 output.
    # Keep that behavior when available.
    for o in json_tx["vout"]:
        if o["value"] == Decimal('100'):
            return o["n"]

    # For protx_register_fund transactions, the deterministic collateral output
    # is the largest positive value output.
    candidates = [o for o in json_tx["vout"] if o["value"] > Decimal('0')]
    assert_greater_than(len(candidates), 0)
    return max(candidates, key=lambda output: output["value"])["n"]
