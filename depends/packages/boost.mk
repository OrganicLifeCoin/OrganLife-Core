...
$(package)_mingw32_thread_model:=$(shell $(host_CXX) -v 2>&1 | sed -n 's/^Thread model: //p' | head -n 1)
$(package)_config_opts_mingw32=target-os=windows binary-format=pe threadapi=$(if $(findstring posix,$($(package)_mingw32_thread_model)),pthread,win32) runtime-link=static
$(package)_cxxflags_mingw32 += $(if $(findstring posix,$($(package)_mingw32_thread_model)),-pthread,)
$(package)_ldflags_mingw32 += $(if $(findstring posix,$($(package)_mingw32_thread_model)),-pthread,)
...
$(package)_config_libraries=chrono,filesystem,system,thread,atomic,test
...
define $(package)_build_cmds
  b2 -d2 -j2 -d1 --prefix=$($(package)_staging_prefix_dir) $($(package)_config_opts) \
    $(foreach lib,$(subst $(comma), ,$($(package)_config_libraries)),--with-$(lib)) \
    toolset=$($(package)_toolset_$(host_os)) stage
endef

define $(package)_stage_cmds
  b2 -d0 -j4 --prefix=$($(package)_staging_prefix_dir) $($(package)_config_opts) \
    $(foreach lib,$(subst $(comma), ,$($(package)_config_libraries)),--with-$(lib)) \
    toolset=$($(package)_toolset_$(host_os)) install
endef
