...
public:
    ...
    void Unserialize(Stream& s)
    {
        ...
        if (size > static_cast<size_t>(std::numeric_limits<int32_t>::max())) {
            throw std::ios_base::failure("bitset size exceeds supported range");
        }
        ...
        while (true) {
            ...
            const size_t idxSize = static_cast<size_t>(idx);
            if (idx < 0 || idxSize >= size) {
                throw std::ios_base::failure("out of bounds index");
            }
            if (last != -1 && idx <= last) {
                throw std::ios_base::failure("offset overflow");
            }
            vec[static_cast<size_t>(idx)] = true;
            last = idx;
        }
    }
};
