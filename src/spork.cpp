...
std::vector<CSporkDef> sporkDefs = {
    ...
    MAKE_SPORK_DEF(SPORK_24_GOVERNANCE_COIN_WEIGHT,         GOVERNANCE_COIN_WEIGHT_DEFAULT),
};
...
namespace {
bool IsAutoPoSEnforcementSpork(const SporkId sporkId)
{
    switch (sporkId) {
...
bool IsPoSActiveAtTip()
{
    const CBlockIndex* tip = GetChainTip();
    if (tip == nullptr) {
        return false;
    }
    return Params().GetConsensus().NetworkUpgradeActive(tip->nHeight, Consensus::UPGRADE_POS);
}
} // namespace
...
// Load persisted spork values on startup.
void CSporkManager::LoadSporksFromDB()
{
    for (const auto& sporkDef : sporkDefs) {
        ...
        if (spork.nSporkID == SPORK_15_NEW_PROTOCOL_ENFORCEMENT_2) {
            ...
            // Ignore stale protocol-enforcement sporks that would disconnect peers during handshake.
...
bool CSporkManager::UpdateSpork(SporkId nSporkID, int64_t nValue)
{
    ...
    {
        LOCK(cs);
        masterPrivKey = strMasterPrivKey;
    }

    if (spork.Sign(masterPrivKey)) {
        spork.Relay();
        AddOrUpdateSporkMessage(spork, true);
        return true;
    }
...
{
    // Keep legacy masternodes enabled permanently for this network rollout.
    // This prevents accidental activation of deterministic-masternode cutoff.
    if (nSporkID == SPORK_21_LEGACY_MNS_MAX_HEIGHT) {
        return false;
    }

    if (IsAutoPoSEnforcementSpork(nSporkID)) {
        bool hasSignedValue = false;
        {
            LOCK(cs);
            hasSignedValue = mapSporksActive.count(nSporkID) != 0;
        }

        // Default behavior: once PoS is active, enforce core network rules
        // even if no spork message was signed yet.
        if (!hasSignedValue && IsPoSActiveAtTip()) {
            return true;
        }
    }
