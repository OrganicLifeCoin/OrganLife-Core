...
/** More lenient ban threshold during IBD */
static const int DEFAULT_BANSCORE_THRESHOLD_IBD = 200;
...
/**
 * Get dynamic ban score threshold based on chain state
 * More lenient during IBD, stricter when synced
 */
int GetDynamicBanScoreThreshold();

/**
 * Runtime staking gate set by ForkGuard during divergence recovery.
 */
