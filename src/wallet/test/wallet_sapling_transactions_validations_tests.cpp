...
static CAmount SumWalletCoinbaseOutputs(CWallet& wallet, int tip, int maturity, bool mature)
{
    CAmount sum = 0;
    for (int h = 1; h <= tip; ++h) {
        const bool isMature = (h <= tip - maturity);
        if (mature != isMature) continue;

        const CBlockIndex* pindex = WITH_LOCK(cs_main, return chainActive[h];);
        BOOST_REQUIRE(pindex != nullptr);
        CBlock block;
        BOOST_REQUIRE(ReadBlockFromDisk(block, pindex));
        const CTransactionRef& coinbaseTx = block.vtx.at(0);
        for (const auto& out : coinbaseTx->vout) {
            if (IsMine(wallet, out.scriptPubKey) != ISMINE_NO) {
                sum += out.nValue;
            }
        }
    }
...
BOOST_AUTO_TEST_CASE(test_in_block_and_mempool_notes_double_spend)
{
    ...
    // Compute expected balances from the actual mined coinbase outputs belonging to the wallet,
    // so the test stays correct even when block rewards are split (e.g. masternode/budget payments).
    const int tip = tipHeight;
    const int maturity = Params().GetConsensus().nCoinbaseMaturity;
    const CAmount expected_available = SumWalletCoinbaseOutputs(*pwalletMain, tip, maturity, true);
    const CAmount expected_immature = SumWalletCoinbaseOutputs(*pwalletMain, tip, maturity, false);
    BOOST_CHECK_EQUAL(pwalletMain->GetAvailableBalance(), expected_available);
    BOOST_CHECK_EQUAL(pwalletMain->GetImmatureBalance(), expected_immature);
