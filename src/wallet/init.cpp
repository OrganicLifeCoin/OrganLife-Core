...
std::string GetWalletHelpString(bool showDebug)
{
    ...
    strUsage += HelpMessageOpt("-wallet=<path>", "Specify wallet database path. Can be specified multiple times to load multiple wallets. Path is interpreted relative to <walletdir> if it is not absolute. Non-existing paths ending in .dat are created as wallet data files in the shared wallet directory, while other non-existing paths are created as directories containing wallet.dat and log files.");
    strUsage += HelpMessageOpt("-walletdir=<dir>", "Specify directory to hold wallets (default: <datadir>/wallets)");
    ...
    strUsage += HelpMessageOpt("-minpowconnections=<n>", "Minimum number of peer connections required to PoW mine when -gen=1 (0 = allow mining with no peers). Useful on low-hash networks to reduce frequent forks/reorgs.");
...
bool WalletVerify()
{
    ...
    const OperationResult migrationResult = MigrateLegacyManagedWalletLayout(migrationWarnings);
    if (!migrationResult) {
        return UIError(migrationResult.getError());
    }
    for (const std::string& warning : migrationWarnings) {
        UIWarning(warning);
    }
