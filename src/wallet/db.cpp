...
BerkeleyEnvironment* GetWalletEnv(const fs::path& wallet_path, std::string& database_filename)
{
    ...
    const bool file_backed_wallet = fs::is_regular_file(wallet_path) ||
                                    (fs::symlink_status(wallet_path).type() == fs::file_not_found &&
                                     wallet_path.has_extension() &&
                                     wallet_path.extension() == ".dat");
    if (file_backed_wallet) {
...
bool BerkeleyEnvironment::Open(bool retry)
{
    ...
    if (!LockDirectory(pathIn, ".walletlock")) {
        LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance of OrganicLife may be using it.\n", strPath);
        return false;
    }
...
BerkeleyBatch::BerkeleyBatch(BerkeleyDatabase& database, const char* pszMode, bool fFlushOnCloseIn) : pdb(nullptr), activeTxn(nullptr)
{
...

    {
        ...
        if (pdb == nullptr) {
            ...
            // OrganicLife from opening the same data file through another
