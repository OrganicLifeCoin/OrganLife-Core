...
struct Params {
    ...
    CAmount nBudgetCycleAmount{0};
    int nBudgetFeeConfirmations;
    int nCoinbaseMaturity;
    int nFutureTimeDriftPoW;
    int nFutureTimeDriftPoS;
    CAmount nMaxMoneyOut;
    CAmount nPremineReward{0};
    ...
    {
        // PoS (TimeV2): 14 seconds (strict) or nFutureTimeDriftPoS (testnet/relaxed)
        if (IsTimeProtocolV2(nHeight)) return (fPowAllowMinDifficultyBlocks ? nFutureTimeDriftPoS : nTimeSlotLength - 1);
