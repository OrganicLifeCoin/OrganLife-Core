...
bool CheckTransaction(const CTransaction& tx, CValidationState& state, bool fColdStakingActive)
{
    ...
    // Size limits
    static_assert(MAX_TX_SIZE_AFTER_SAPLING < MAX_BLOCK_SIZE_CURRENT, "Max block size must be bigger than max TX size");    // sanity
    if (tx.GetTotalSize() > MAX_TX_SIZE_AFTER_SAPLING) {
        return state.DoS(10, error("tx oversize: %d > %d", tx.GetTotalSize(), MAX_TX_SIZE_AFTER_SAPLING), REJECT_INVALID, "bad-txns-oversize");
    }
