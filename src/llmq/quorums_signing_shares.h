...
private:
    ...
    uint32_t GetOrCreateSendSessionId(NodeId nodeId, const uint256& signHash, std::vector<CSigSesAnn>& sigSessionAnnouncements) EXCLUSIVE_LOCKS_REQUIRED(cs);

private:
    bool GetSessionInfoByRecvId(NodeId nodeId, uint32_t sessionId, CSigSharesNodeState::SessionInfo& retInfo) LOCKS_EXCLUDED(cs);
    ...
    void CollectSigSharesToSend(std::unordered_map<NodeId, std::vector<CSigShare>>& sigSharesToSend, const std::vector<CNode*>& vNodes) EXCLUSIVE_LOCKS_REQUIRED(cs);
