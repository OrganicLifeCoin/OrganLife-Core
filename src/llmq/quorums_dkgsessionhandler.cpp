...
bool CDKGSessionHandler::InitNewQuorum(const CBlockIndex* pindexQuorum)
{
    ...
    auto newSession = std::make_shared<CDKGSession>(params, blsWorker, dkgManager);
    if (!newSession->Init(pindexQuorum, mns, activeMasternodeManager->GetProTx())) {
        LogPrintf("CDKGSessionHandler::%s -- quorum initialization failed for %s\n", __func__, params.name);
        return false;
    }

    curSession = std::move(newSession);
