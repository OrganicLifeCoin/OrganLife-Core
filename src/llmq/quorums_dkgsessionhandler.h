...
private:
    ...
    uint256 quorumHash GUARDED_BY(cs){UINT256_ZERO};
    // Owned by the phase handler thread after construction. This is intentionally not guarded by cs.
...
private:
    ...
    QuorumPhaseAndHash GetPhaseAndQuorumHash() const LOCKS_EXCLUDED(cs);
