...
{
    {
        ...
        for (const auto& entry : nodeStates) {
            shuffledNodeIds.emplace_back(entry.first);
        }
        ...
        for (auto nodeId : shuffledNodeIds) {
            ...
            if (nodeState.pendingIncomingSigShares.Empty()) {
                continue;
            }

            auto& sigShare = *nodeState.pendingIncomingSigShares.GetFirst();
            if (!sigShares.Has(sigShare.GetKey())) {
                uniqueSignHashes.emplace(nodeId, sigShare.GetSignHash());
                retSigShares[nodeId].emplace_back(sigShare);
            }
            nodeState.pendingIncomingSigShares.Erase(sigShare.GetKey());
        }
...
void CSigSharesManager::TryRecoverSig(const CQuorumCPtr& quorum, const uint256& id, const uint256& msgHash, CConnman& connman)
{
    ...
    std::vector<CBLSId> idsForRecovery;
    {
        ...
        for (auto it = sigShares->begin(); it != sigShares->end() && sigSharesForRecovery.size() < static_cast<size_t>(quorum->params.threshold); ++it) {
...
{
    assert(attempt >= 0 && static_cast<size_t>(attempt) < quorum->members.size());
...
void CSigSharesManager::CollectSigSharesToAnnounce(std::unordered_map<NodeId, std::unordered_map<uint256, CSigSharesInv, StaticSaltedHasher>>& sigSharesToAnnounce)
{
    ...
    std::vector<SigShareKey> sigShareKeys;

    this->sigSharesToAnnounce.ForEach([&](const SigShareKey& sigShareKey, bool) {
        sigShareKeys.emplace_back(sigShareKey);
    });

    for (const auto& sigShareKey : sigShareKeys) {
...
uint32_t CSigSharesManager::GetOrCreateSendSessionId(NodeId nodeId, const uint256& signHash, std::vector<CSigSesAnn>& sigSessionAnnouncements)
{
    auto& nodeState = nodeStates[nodeId];
    ...
    if (session->sendSessionId == (uint32_t)-1) {
        ...
        sigSessionAnnouncements.emplace_back(sigSesAnn);
    }
...

    {
        ...
        for (auto& p : sigSharesToRequest) {
            for (auto& p2 : p.second) {
                p2.second.sessionId = GetOrCreateSendSessionId(p.first, p2.first, sigSessionAnnouncements[p.first]);
            }
        }
...
void CSigSharesManager::Cleanup()
{
    ...
    std::unordered_set<NodeId> nodeStatesToDelete;
    {
        LOCK(cs);
        for (const auto& entry : nodeStates) {
            nodeStatesToDelete.emplace(entry.first);
        }
    }
    ...
    for (auto nodeId : nodeStatesToDelete) {
        auto& nodeState = nodeStates[nodeId];
        std::vector<SigShareKey> requestedKeys;
