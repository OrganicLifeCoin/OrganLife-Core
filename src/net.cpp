...
static constexpr int64_t NET_STALL_AUTO_RECOVER_TIMEOUT_US = 120 * 1000000;
static constexpr int64_t NET_STALL_AUTO_RECOVER_COOLDOWN_US = 60 * 1000000;
static constexpr int64_t NET_STALL_NETWORK_FLIP_WINDOW_US = 2 * 1000000;
...
RecursiveMutex cs_mapAlreadyAskedFor;
limitedmap<CInv, int64_t> mapAlreadyAskedFor(MAX_INV_SZ);

ConnmanThreadStallEvent ConnmanThreadGetStallEvent(
    int64_t now_us,
    int64_t last_heartbeat_us,
    int64_t stall_timeout_us,
    bool warning_already_logged)
{
    if (stall_timeout_us <= 0 || now_us <= 0 || last_heartbeat_us <= 0) {
        return ConnmanThreadStallEvent::NONE;
    }

    const int64_t age_us = now_us - last_heartbeat_us;
    if (age_us >= stall_timeout_us) {
        return warning_already_logged ? ConnmanThreadStallEvent::NONE : ConnmanThreadStallEvent::STALLED;
    }

    return warning_already_logged ? ConnmanThreadStallEvent::RECOVERED : ConnmanThreadStallEvent::NONE;
}

bool ConnmanThreadShouldAutoRecover(
    int64_t now_us,
    int64_t last_heartbeat_us,
    int64_t recover_timeout_us,
    int64_t last_recovery_us,
    int64_t recover_cooldown_us)
{
    if (recover_timeout_us <= 0 || now_us <= 0 || last_heartbeat_us <= 0) {
        return false;
    }

    if (now_us - last_heartbeat_us < recover_timeout_us) {
        return false;
    }

    if (last_recovery_us > 0 && recover_cooldown_us > 0 && now_us - last_recovery_us < recover_cooldown_us) {
        return false;
    }
...
void CConnman::ClearBanned()
{
    ...
    RequestDumpBanlist(); // store banlist to disk (async, avoids blocking RPC threads)
    if(clientInterface)
...
void CConnman::RequestDumpBanlist()
{
    bool expected = false;
    if (!fDumpBanlistPending.compare_exchange_strong(expected, true)) {
        return;
    }
    WakeMessageHandler();
}
...
void CNode::copyStats(CNodeStats& stats, const std::vector<bool>& m_asmap)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (OrganicLife users should be well used to small numbers with many decimal places by now :)
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket) {
...

    {
        ...
        if (!lockMain) {
            LogPrint(BCLog::NET, "AcceptConnection: cs_main held, deferring connection from %s\n", addr.ToString());
            pnode->Release();
            delete pnode;
            return;
        }
...
void CConnman::DisconnectNodes()
{
    ...
    while (true) {
        CNode* nodeToDelete = nullptr;
        {
            LOCK(cs_vNodes);
            for (auto it = vNodesDisconnected.begin(); it != vNodesDisconnected.end(); ++it) {
                CNode* pnode = *it;
        ...
        // DeleteNode calls m_msgproc->FinalizeNode which locks cs_main.
        // If cs_main is held by another thread, defer deletion to avoid blocking the net thread.
        extern RecursiveMutex cs_main;
        TRY_LOCK(cs_main, lockMain);
        if (!lockMain) {
            LOCK(cs_vNodes);
            vNodesDisconnected.push_back(nodeToDelete);
            break;
        }

        DeleteNode(nodeToDelete);
        MarkThreadHeartbeat(m_threadHeartbeatNet);
    }
}
...
void CConnman::NotifyNumConnectionsChanged()
{
    ...
    if (vNodesSize != nPrevNodeCount) {
        ...
        if (clientInterface) clientInterface->NotifyNumConnectionsChanged((int)vNodesSize);
    }
}
...
{
    SetThreadHeartbeatActive(m_threadHeartbeatNet, true);
    while (!interruptNet) {
        MarkThreadHeartbeat(m_threadHeartbeatNet);
        DisconnectNodes();
...
void CConnman::SetThreadHeartbeatActive(ThreadHeartbeatState& state, bool active)
{
    state.active.store(active, std::memory_order_relaxed);
    state.stalled_warning_logged.store(false, std::memory_order_relaxed);
    state.last_heartbeat_us.store(active ? GetTimeMicros() : 0, std::memory_order_relaxed);
}

void CConnman::MarkThreadHeartbeat(ThreadHeartbeatState& state)
{
    if (!state.active.load(std::memory_order_relaxed)) {
        return;
    }

    state.last_heartbeat_us.store(GetTimeMicros(), std::memory_order_relaxed);
}

void CConnman::CheckThreadStall(ThreadHeartbeatState& state, int64_t now_us)
{
    if (!state.active.load(std::memory_order_relaxed)) {
        return;
    }

    const int64_t last_heartbeat_us = state.last_heartbeat_us.load(std::memory_order_relaxed);
    const bool warned = state.stalled_warning_logged.load(std::memory_order_relaxed);
    const ConnmanThreadStallEvent event = ConnmanThreadGetStallEvent(
        now_us,
        last_heartbeat_us,
        state.stall_timeout_us,
        warned);

    if (event == ConnmanThreadStallEvent::STALLED) {
        state.stalled_warning_logged.store(true, std::memory_order_relaxed);
        const int stalled_for_sec = static_cast<int>((now_us - last_heartbeat_us) / 1000000);
        const int threshold_sec = static_cast<int>(state.stall_timeout_us / 1000000);
        LogPrintf("WARNING: CConnman thread '%s' heartbeat stalled (%d sec without heartbeat, threshold=%d sec)\n",
                  state.name, stalled_for_sec, threshold_sec);
        return;
    }

    if (event == ConnmanThreadStallEvent::RECOVERED) {
        state.stalled_warning_logged.store(false, std::memory_order_relaxed);
        const int age_ms = static_cast<int>((now_us - last_heartbeat_us) / 1000);
        LogPrintf("CConnman thread '%s' heartbeat recovered (age=%d ms)\n", state.name, age_ms);
    }
}

void CConnman::MaybeAutoRecoverNetThread(int64_t now_us)
{
    if (!m_netStallAutoRecoverEnabled.load(std::memory_order_relaxed)) {
        return;
    }

    const int64_t last_heartbeat_us = m_threadHeartbeatNet.last_heartbeat_us.load(std::memory_order_relaxed);
    const int64_t last_recovery_us = m_netStallLastRecoveryUs.load(std::memory_order_relaxed);
    if (!ConnmanThreadShouldAutoRecover(now_us,
                                        last_heartbeat_us,
                                        NET_STALL_AUTO_RECOVER_TIMEOUT_US,
                                        last_recovery_us,
                                        NET_STALL_AUTO_RECOVER_COOLDOWN_US)) {
        return;
    }

    m_netStallLastRecoveryUs.store(now_us, std::memory_order_relaxed);
    LogPrintf("WARNING: CConnman auto-recovery triggered for 'net' thread stall\n");

    // Wake network wait points and force a short network reset cycle.
    WakeSelect();
    WakeMessageHandler();
    SetNetworkActive(false);
    m_netStallNetworkReenableUs.store(now_us + NET_STALL_NETWORK_FLIP_WINDOW_US, std::memory_order_relaxed);
}

void CConnman::MaybeReenableNetworkAfterRecovery(int64_t now_us)
{
    if (!m_netStallAutoRecoverEnabled.load(std::memory_order_relaxed)) {
        return;
    }

    const int64_t reenable_us = m_netStallNetworkReenableUs.load(std::memory_order_relaxed);
    if (reenable_us <= 0 || now_us < reenable_us) {
        return;
    }

    m_netStallNetworkReenableUs.store(0, std::memory_order_relaxed);
    SetNetworkActive(true);
    LogPrintf("CConnman auto-recovery re-enabled network activity after net-thread stall\n");
}

void CConnman::ThreadStallMonitor()
{
    while (!interruptNet) {
        const int64_t now_us = GetTimeMicros();
        CheckThreadStall(m_threadHeartbeatNet, now_us);
        CheckThreadStall(m_threadHeartbeatOpenCon, now_us);
        CheckThreadStall(m_threadHeartbeatAddCon, now_us);
        CheckThreadStall(m_threadHeartbeatMsgProc, now_us);
        MaybeAutoRecoverNetThread(now_us);
        MaybeReenableNetworkAfterRecovery(now_us);
...
{
    SetThreadHeartbeatActive(m_threadHeartbeatOpenCon, true);
    MarkThreadHeartbeat(m_threadHeartbeatOpenCon);
...
{
    SetThreadHeartbeatActive(m_threadHeartbeatAddCon, true);
    MarkThreadHeartbeat(m_threadHeartbeatAddCon);
...
void CConnman::ThreadMessageHandler()
{
    ...
    SetThreadHeartbeatActive(m_threadHeartbeatMsgProc, true);

    while (!flagInterruptMsgProc) {
        MarkThreadHeartbeat(m_threadHeartbeatMsgProc);
        std::vector<CNode*> vNodesCopy = CopyNodeVector();
        ...
        if (fDumpBanlistPending.exchange(false)) {
            DumpBanlist();
        }

        WAIT_LOCK(mutexMsgProc, lock);
        if (!fMoreWork) {
            const auto wakeDeadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(100);
            while (!fMsgProcWake) {
                if (condMsgProc.wait_until(lock, wakeDeadline) == std::cv_status::timeout) {
                    break;
                }
            }
        }
...
bool CConnman::Start(CScheduler& scheduler, const Options& connOptions)
{
    ...
    interruptNet.reset();
    flagInterruptMsgProc = false;
    m_netStallAutoRecoverEnabled.store(!Params().IsRegTestNet(), std::memory_order_relaxed);
    m_netStallLastRecoveryUs.store(0, std::memory_order_relaxed);
    m_netStallNetworkReenableUs.store(0, std::memory_order_relaxed);

    SetThreadHeartbeatActive(m_threadHeartbeatNet, false);
    SetThreadHeartbeatActive(m_threadHeartbeatOpenCon, false);
    SetThreadHeartbeatActive(m_threadHeartbeatAddCon, false);
    SetThreadHeartbeatActive(m_threadHeartbeatMsgProc, false);
    ...
    threadStallMonitor = std::thread(&TraceThread<std::function<void()> >, "netwatch", std::function<void()>(std::bind(&CConnman::ThreadStallMonitor, this)));
...
void CConnman::JoinThreadWithProgress(std::thread& thread, const char* name, ThreadHeartbeatState* heartbeat)
{
    if (!thread.joinable()) {
        return;
    }

    if (heartbeat && heartbeat->active.load(std::memory_order_relaxed)) {
        const int64_t now_us = GetTimeMicros();
        const int64_t last_heartbeat_us = heartbeat->last_heartbeat_us.load(std::memory_order_relaxed);
        if (last_heartbeat_us > 0 && now_us >= last_heartbeat_us) {
            const int age_ms = static_cast<int>((now_us - last_heartbeat_us) / 1000);
            LogPrintf("CConnman::Stop: joining thread '%s' (last heartbeat %d ms ago)\n", name, age_ms);
        } else {
            LogPrintf("CConnman::Stop: joining thread '%s' (heartbeat unavailable)\n", name);
        }
    } else {
        LogPrintf("CConnman::Stop: joining thread '%s'\n", name);
    }

    thread.join();
    LogPrintf("CConnman::Stop: joined thread '%s'\n", name);
}

void CConnman::Stop()
{
    JoinThreadWithProgress(threadMessageHandler, "msghand", &m_threadHeartbeatMsgProc);
    JoinThreadWithProgress(threadOpenConnections, "opencon", &m_threadHeartbeatOpenCon);
    JoinThreadWithProgress(threadOpenAddedConnections, "addcon", &m_threadHeartbeatAddCon);
    JoinThreadWithProgress(threadDNSAddressSeed, "dnsseed");
    JoinThreadWithProgress(threadSocketHandler, "net", &m_threadHeartbeatNet);
    JoinThreadWithProgress(threadStallMonitor, "netwatch");
    ...
    std::vector<CNode*> nodesToDelete;
    std::list<CNode*> nodesDisconnectedToDelete;
    {
        LOCK(cs_vNodes);
        nodesToDelete = vNodes;
        nodesDisconnectedToDelete = vNodesDisconnected;
        vNodes.clear();
        vNodesDisconnected.clear();
    }

    // Close sockets
    for (CNode* pnode : nodesToDelete)
        pnode->CloseSocketDisconnect();
    ...
    for (CNode* pnode : nodesDisconnectedToDelete) {
        DeleteNode(pnode);
    }
...
{
    vstats.clear();
    const std::vector<CNode*> vNodesCopy = CopyNodeVector();
    vstats.reserve(vNodesCopy.size());
    for (CNode* pnode : vNodesCopy) {
        vstats.emplace_back();
...
{
    LOCK2(cs_mapAlreadyAskedFor, cs_inventory);
