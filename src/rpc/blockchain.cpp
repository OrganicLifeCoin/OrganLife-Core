...
UniValue getsupplyinfo(const JSONRPCRequest& request)
{
    ...
    if (fForceUpdate) {
        ...
        FlushStateToDisk();
        // Refresh money supply cache even during IBD.
        // This RPC is explicitly user-invoked, and the help text promises an updated value.
        if (pcoinsTip) {
        ...
        // Testnet can be low-activity, which may delay periodic chainstate flushes.
        // If the cache hasn't been initialized yet, initialize it on-demand once.
        if (pcoinsTip && MoneySupply.GetCacheHeight() == 0) {
            const int chainHeight = WITH_LOCK(cs_main, return chainActive.Height());
            if (chainHeight > 0) {
                MoneySupply.Update(pcoinsTip->GetTotalAmount(), chainHeight);
            }
...
static std::string NetworkUpgradeDisplayName(const std::string& rawName)
{
        ...
        int height)
{
    ...
    if (consensusParams.vUpgrades[idx].nActivationHeight != Consensus::NetworkUpgrade::NO_ACTIVATION_HEIGHT) {
        std::string name = NetworkUpgradeDisplayName(NetworkUpgradeInfo[idx].strName);
