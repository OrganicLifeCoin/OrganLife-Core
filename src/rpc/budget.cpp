                       ...
                       int &nPaymentCount, int &nBlockStart, CTxDestination &address, CAmount &nAmount)
{
    ...
    if (!IsValidDestination(address))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid OrganicLife address");
...
UniValue preparebudget(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 6)
        throw std::runtime_error(
            ...
            "5. \"address\":     (string, required) OrganicLife address to send payments to\n"
...
static uint8_t ParseGovVoteDirection(const std::string& strVote)
{
    if (strVote == "yes") return CGovVoteCastTx::VOTE_YES;
    if (strVote == "no") return CGovVoteCastTx::VOTE_NO;
    throw JSONRPCError(RPC_INVALID_PARAMETER, "vote must be 'yes' or 'no'");
}

static void EnsureGovVotingActiveAtNextBlock()
{
    const CBlockIndex* tip = GetChainTip();
    if (!tip) {
        throw JSONRPCError(RPC_IN_WARMUP, "Try again after active chain is loaded");
    }
    if (!Params().GetConsensus().NetworkUpgradeActive(tip->nHeight + 1, Consensus::UPGRADE_V6_1_GOV)) {
        throw JSONRPCError(RPC_MISC_ERROR,
                           strprintf("Governance coin voting is not active at height %d", tip->nHeight + 1));
    }
}

static void EnsureGovVotingFullySynced()
{
    if (!g_tiertwo_sync_state.IsSynced()) {
        LogPrintf("%s: sync not finished (phase=%d, blockchainSynced=%d)\n", __func__,
                  g_tiertwo_sync_state.GetSyncPhase(), g_tiertwo_sync_state.IsBlockchainSynced() ? 1 : 0);
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Masternode/Budget sync not finished yet");
    }
}

static void EnsureGovProposalVotingWindowOpenAtNextBlock(const uint256& proposalHash)
{
    const CBlockIndex* tip = GetChainTip();
    if (!tip) {
        throw JSONRPCError(RPC_IN_WARMUP, "Try again after active chain is loaded");
    ...
    if (!g_budgetman.GetProposal(proposalHash, proposal)) {
        throw JSONRPCError(
                ...
                "unknown proposal hash (not found on this node)");
    }

    const int nextHeight = tip->nHeight + 1;
    if (proposal.GetBlockEnd() < nextHeight) {
        throw JSONRPCError(
                RPC_INVALID_PARAMETER,
                strprintf("proposal voting window closed at height %d", proposal.GetBlockEnd()));
    }
}

static COutPoint ParseGovLockRef(const std::string& lockRef)
{
    const std::size_t sepPos = lockRef.find(':');
    if (sepPos == std::string::npos || sepPos == 0 || sepPos == lockRef.size() - 1) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid lock ref '%s', expected txid:vout", lockRef));
    }

    const std::string txidHex = lockRef.substr(0, sepPos);
    if (!IsHex(txidHex) || txidHex.size() != 64) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid txid in lock ref '%s'", lockRef));
    }

    uint64_t outputIndex{0};
    try {
        outputIndex = std::stoull(lockRef.substr(sepPos + 1));
    } catch (const std::exception&) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("Invalid vout index in lock ref '%s'", lockRef));
    }
    if (outputIndex > std::numeric_limits<uint32_t>::max()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("vout index out of range in lock ref '%s'", lockRef));
    }

    return COutPoint(uint256S(txidHex), static_cast<uint32_t>(outputIndex));
}

#ifdef ENABLE_WALLET

template <typename SpecialTxPayload>
static void FundGovSpecialTx(CWallet* pwallet, CMutableTransaction& tx, SpecialTxPayload& payload)
{
    SetTxPayload(tx, payload);

    static CTxOut dummyTxOut(0, CScript() << OP_RETURN);
    const bool dummyAdded = tx.vout.empty();
    if (dummyAdded) {
    ...
    CAmount nFee{0};
    CFeeRate feeRate(0);
    int nChangePos = tx.vout.size(); // keep existing outputs in place
    std::string strFailReason;
    if (!pwallet->FundTransaction(tx, nFee, false, feeRate, nChangePos, strFailReason, false, false, {})) {
        throw JSONRPCError(RPC_WALLET_ERROR, strFailReason);
    }

    if (dummyAdded && tx.vout.size() > 1) {
        const auto it = std::find(tx.vout.begin(), tx.vout.end(), dummyTxOut);
        if (it != tx.vout.end()) {
            tx.vout.erase(it);
        }
    }
...
static void SignGovSpecialTx(CWallet* pwallet, CMutableTransaction& tx)
{
    LOCK2(cs_main, pwallet->cs_wallet);
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (coin.IsSpent()) {
            throw JSONRPCError(RPC_WALLET_ERROR, strprintf("Input %s not found or already spent", txin.prevout.ToStringShort()));
        }

        txin.scriptSig.clear();
        SignatureData sigdata;
        if (!ProduceSignature(MutableTransactionSignatureCreator(pwallet, &tx, i, coin.out.nValue, SIGHASH_ALL),
                              coin.out.scriptPubKey, sigdata, tx.GetRequiredSigVersion(), false)) {
            throw JSONRPCError(RPC_WALLET_ERROR,
                               strprintf("Failed to sign input %d (%s)", i, txin.prevout.ToStringShort()));
        }
        UpdateTransaction(tx, i, sigdata);
    }
}

static std::string CommitGovSpecialTx(CWallet* pwallet, CMutableTransaction& tx)
{
    ...
    SignGovSpecialTx(pwallet, tx);

    const CWallet::CommitResult& res =
            pwallet->CommitTransaction(MakeTransactionRef(tx), (CReserveKey*)nullptr, g_connman.get());
    if (res.status != CWallet::CommitStatus::OK) {
        throw JSONRPCError(RPC_WALLET_ERROR, res.ToString());
    }
...
static CKeyID GetOwnerKeyIdForLock(const COutPoint& lockRef, const uint256& proposalHash, bool verifyProposal)
{
    CGovVoteLockRecord lockRecord;
    if (governanceVoteIndex && governanceVoteIndex->GetLockRecord(lockRef, lockRecord)) {
        if (verifyProposal && lockRecord.proposalHash != proposalHash) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "lock ref points to different proposal");
        }
        return lockRecord.ownerKeyId;
    }

    CTransactionRef lockTx;
    uint256 lockHashBlock;
    if (!GetTransaction(lockRef.hash, lockTx, lockHashBlock, true)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, strprintf("lock tx %s not found", lockRef.hash.ToString()));
    }
    if (lockTx->nType != CTransaction::TxType::GOVVOTELOCK) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "referenced lock tx is not a GOVVOTELOCK transaction");
    }

    CGovVoteLockTx lockPayload;
    if (!GetTxPayload(*lockTx, lockPayload)) {
        throw JSONRPCError(RPC_DESERIALIZATION_ERROR, "failed to parse lock tx payload");
    }
    if (verifyProposal && lockPayload.proposalHash != proposalHash) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "lock ref points to different proposal");
    }

    return lockPayload.ownerKeyId;
}

static uint32_t FindGovLockOutputIndex(const CTransaction& tx, const CGovVoteLockTx& payload)
{
...
UniValue mnbudgetvote(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || (request.params.size() == 3 && (strCommand != "local" && strCommand != "many")) || (request.params.size() == 4 && strCommand != "alias") ||
        ...
        throw std::runtime_error(
            "mnbudgetvote \"local|many|alias\" \"hash\" \"yes|no\" ( \"alias\" )\n"
            "\nVote on a budget proposal\n"
            "\nThe deterministic masternode system is used."

            "\nArguments:\n"
            "1. \"mode\"      (string, required) The voting mode. 'many' for voting with a MN controller and casting the same vote for each MN, 'alias' for voting with a MN controller and casting a vote for a single MN\n"
            "2. \"hash\"      (string, required) The budget proposal hash\n"
            "3. \"votecast\"  (string, required) Your vote. 'yes' to vote for the proposal, 'no' to vote against\n"
            "4. \"alias\"     (string, required for 'alias' mode) The MN alias to cast a vote for (the hash of the proTx transaction).\n"
    ...
    if (strCommand == "many" || (isAlias = strCommand == "alias")) {
        ...
        return mnBudgetVoteInner(pwallet, hash, false, nVote, mnAlias);
    }
...
UniValue creategovvotelock(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 4) {
        throw std::runtime_error(
                "creategovvotelock \"proposal_hash\" amount unlock_height ( return_lock_ref )\n"
                "\nCreate, sign and broadcast a governance vote lock transaction.\n"
                "\nArguments:\n"
                "1. \"proposal_hash\"  (string, required) Proposal hash the lock belongs to\n"
                "2. amount            (numeric, required) Amount to lock in whole PIV units\n"
                "3. unlock_height     (numeric, required) Unlock height\n"
                "4. return_lock_ref   (boolean, optional, default=false) Return lock reference object instead of txid string\n"
                "\nResult:\n"
                "\"txid\"              (string) Transaction id of the lock transaction (default)\n"
                "or\n"
                "{\n"
                "  \"txid\": \"...\",         (string) Transaction id of the lock transaction\n"
                "  \"vout\": n,              (numeric) Output index of the lock output\n"
                "  \"outpoint\": \"txid:n\",  (string) Lock reference for castgovvote\n"
                "  \"proposal_hash\": \"...\",(string) Proposal hash bound to this lock\n"
                "  \"amount\": n,            (numeric) Locked amount (whole PIV)\n"
                "  \"unlock_height\": n      (numeric) Unlock height\n"
                "}\n");
    }

    EnsureGovVotingFullySynced();
    EnsureGovVotingActiveAtNextBlock();
    EnsureWalletIsUnlocked(pwallet);

    const uint256 proposalHash = ParseHashV(request.params[0], "proposal_hash");
    EnsureGovProposalVotingWindowOpenAtNextBlock(proposalHash);
    const CAmount lockAmount = AmountFromValue(request.params[1]);
    const uint32_t unlockHeight = request.params[2].get_int();
    const bool returnLockRef = request.params.size() == 4 && request.params[3].get_bool();

    CTxDestination ownerDest;
    {
        LOCK(pwallet->cs_wallet);
        auto ownerDestResult = pwallet->getNewAddress("gov-vote-lock");
        if (!ownerDestResult) {
            throw JSONRPCError(RPC_WALLET_ERROR, ownerDestResult.getError());
        }
        ownerDest = *ownerDestResult.getObjResult();
    }

    const CKeyID* ownerKeyId = boost::get<CKeyID>(&ownerDest);
    if (!ownerKeyId) {
        throw JSONRPCError(RPC_WALLET_ERROR, "failed to derive owner key for lock transaction");
    }

    CMutableTransaction tx;
    tx.nVersion = CTransaction::TxVersion::SAPLING;
    tx.nType = CTransaction::TxType::GOVVOTELOCK;
    tx.vout.emplace_back(lockAmount, GetScriptForDestination(ownerDest));

    CGovVoteLockTx payload;
    payload.proposalHash = proposalHash;
    payload.lockAmount = lockAmount;
    payload.unlockHeight = unlockHeight;
    payload.ownerKeyId = *ownerKeyId;

    CValidationState payloadState;
    if (!payload.IsTriviallyValid(payloadState)) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, FormatStateMessage(payloadState));
    }

    FundGovSpecialTx(pwallet, tx, payload);
    const uint32_t lockVout = FindGovLockOutputIndex(CTransaction(tx), payload);
    const uint256 lockTxHash = tx.GetHash();
    const std::string txid = CommitGovSpecialTx(pwallet, tx);
    {
        LOCK(pwallet->cs_wallet);
        pwallet->LockCoin(COutPoint(lockTxHash, lockVout));
    }
    if (!returnLockRef) {
        return txid;
    }
    ...
    ret.pushKV("vout", static_cast<int64_t>(lockVout));
    ret.pushKV("outpoint", strprintf("%s:%d", txid, lockVout));
    ret.pushKV("proposal_hash", proposalHash.ToString());
    ret.pushKV("amount", ValueFromAmount(lockAmount));
    ret.pushKV("unlock_height", static_cast<int64_t>(unlockHeight));
    return ret;
#endif
}
...
UniValue castgovvote(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 3) {
        throw std::runtime_error(
                "castgovvote \"proposal_hash\" \"yes|no\" [\"txid:vout\", ...]\n"
                "\nCreate, sign and broadcast a governance vote cast transaction.\n"
                "\nArguments:\n"
                "1. \"proposal_hash\"  (string, required) Proposal hash\n"
                "2. \"yes|no\"         (string, required) Vote direction\n"
                "3. \"lock_refs\"      (array, required) Lock outpoints to vote with\n"
                "\nResult:\n"
                "\"txid\"              (string) Transaction id of the cast transaction\n");
    }
    ...
    const uint256 proposalHash = ParseHashV(request.params[0], "proposal_hash");
    EnsureGovProposalVotingWindowOpenAtNextBlock(proposalHash);
    const uint8_t voteDirection = ParseGovVoteDirection(request.params[1].get_str());
    const UniValue refsArray = request.params[2].get_array();
    if (refsArray.empty()) {
        throw JSONRPCError(RPC_INVALID_PARAMETER, "lock_refs must not be empty");
    }

    std::vector<COutPoint> lockRefs;
    lockRefs.reserve(refsArray.size());
    for (size_t i = 0; i < refsArray.size(); ++i) {
        lockRefs.emplace_back(ParseGovLockRef(refsArray[i].get_str()));
    }

    CKeyID ownerKeyId;
    bool ownerSet = false;
    for (const auto& lockRef : lockRefs) {
        const CKeyID lockOwner = GetOwnerKeyIdForLock(lockRef, proposalHash, true);
        if (!ownerSet) {
            ownerKeyId = lockOwner;
            ownerSet = true;
        } else if (ownerKeyId != lockOwner) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "all lock refs must belong to the same owner key");
        }
    }
    ...
    CKey ownerKey;
    {
        ...
        if (!pwallet->GetKey(ownerKeyId, ownerKey)) {
            throw JSONRPCError(RPC_WALLET_ERROR, "wallet does not have the private key for the provided lock refs");
        }
    }

    CMutableTransaction tx;
    tx.nVersion = CTransaction::TxVersion::SAPLING;
    tx.nType = CTransaction::TxType::GOVVOTECAST;

    CGovVoteCastTx payload;
    payload.proposalHash = proposalHash;
    payload.voteDirection = voteDirection;
    payload.lockRefs = lockRefs;
    if (!CHashSigner::SignHash(payload.GetSignatureHash(), ownerKey, payload.sig)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "failed to sign governance cast payload");
    }
...

    {
        LOCK(pwallet->cs_wallet);
        for (const auto& lockRef : lockRefs) {
            pwallet->LockCoin(lockRef);
        }
    }

    FundGovSpecialTx(pwallet, tx, payload);
    return CommitGovSpecialTx(pwallet, tx);
#endif
}
...
UniValue listgovlocks(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() > 1) {
        throw std::runtime_error(
                "listgovlocks ( \"proposal_hash\" )\n"
                "\nList wallet governance lock transactions.\n"
                "\nArguments:\n"
                "1. \"proposal_hash\"  (string, optional) Filter by proposal hash\n"
                "\nResult:\n"
                "[ {\"txid\":..., \"outpoint\":..., \"proposal_hash\":..., \"amount\":..., \"unlock_height\":..., \"confirmations\":..., \"used\":...}, ... ]\n");
    }

    Optional<uint256> proposalFilter = nullopt;
    if (request.params.size() == 1) {
        proposalFilter = ParseHashV(request.params[0], "proposal_hash");
    }

    UniValue ret(UniValue::VARR);
    LOCK2(cs_main, pwallet->cs_wallet);
    for (const auto& [txid, wtx] : pwallet->mapWallet) {
        if (!wtx.tx || wtx.tx->nType != CTransaction::TxType::GOVVOTELOCK) continue;

        CGovVoteLockTx lockPayload;
        if (!GetTxPayload(*wtx.tx, lockPayload)) continue;
        if (proposalFilter && lockPayload.proposalHash != *proposalFilter) continue;

        const uint32_t lockVout = FindGovLockOutputIndex(*wtx.tx, lockPayload);
        const COutPoint lockRef(txid, lockVout);
        UniValue obj(UniValue::VOBJ);
        obj.pushKV("txid", txid.ToString());
        obj.pushKV("vout", static_cast<int64_t>(lockVout));
        obj.pushKV("outpoint", strprintf("%s:%d", txid.ToString(), lockVout));
        obj.pushKV("proposal_hash", lockPayload.proposalHash.ToString());
        obj.pushKV("amount", lockPayload.lockAmount / COIN);
        obj.pushKV("unlock_height", static_cast<int64_t>(lockPayload.unlockHeight));
        obj.pushKV("owner", EncodeDestination(lockPayload.ownerKeyId));
        obj.pushKV("confirmations", wtx.GetDepthInMainChain());

        bool used = false;
        if (governanceVoteIndex) {
            used = governanceVoteIndex->IsLockUsedForProposal(lockRef, lockPayload.proposalHash);
        }
        obj.pushKV("used", used);
        ret.push_back(obj);
    }
...
UniValue getgovvotestatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1) {
        throw std::runtime_error(
                "getgovvotestatus \"proposal_hash\"\n"
                "\nReturn combined governance vote status for a proposal.\n"
                "\nArguments:\n"
                "1. \"proposal_hash\"  (string, required) Proposal hash\n"
                "\nResult:\n"
                "{ \"mn_yes\":n, \"mn_no\":n, \"coin_yes\":n, \"coin_no\":n, \"k\":n.n, \"combined_score\":n.n, \"cutoff_height\":n }\n");
    }
    ...
    int64_t coinYes = 0;
    int64_t coinNo = 0;
    int64_t cutoffHeight = -1;
    CBudgetProposal budgetProposal;
    if (g_budgetman.GetProposal(proposalHash, budgetProposal)) {
        mnYes = budgetProposal.GetYeas();
        mnNo = budgetProposal.GetNays();
        coinYes = budgetProposal.GetCoinYeas();
        coinNo = budgetProposal.GetCoinNays();
        cutoffHeight = budgetProposal.GetBlockEnd();
    } else {
        g_budgetman.GetCoinVoteTotals(proposalHash, coinYes, coinNo);
    }

    const int64_t kFixed = g_budgetman.GetCoinGovernanceWeightFixed();
    const int64_t combinedFixed =
            CBudgetProposal::ComputeCombinedScore(mnYes - mnNo, coinYes - coinNo, kFixed);

    UniValue ret(UniValue::VOBJ);
    ret.pushKV("mn_yes", mnYes);
    ret.pushKV("mn_no", mnNo);
    ret.pushKV("coin_yes", coinYes);
    ret.pushKV("coin_no", coinNo);
    ret.pushKV("k", static_cast<double>(kFixed) / CBudgetProposal::HYBRID_SCORE_SCALE);
    ret.pushKV("combined_score", static_cast<double>(combinedFixed) / CBudgetProposal::HYBRID_SCORE_SCALE);
    ret.pushKV("cutoff_height", cutoffHeight);
    return ret;
}
...
UniValue getbudgetprojection(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) OrganicLife address of payment\n"
...
UniValue mnbudgetrawvote(const JSONRPCRequest& request)
{
    ...
    auto dmn = mnList.GetMNByCollateral(vin.prevout);
    if (!dmn) {
...
UniValue mnfinalbudget(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp ||
        ...
        throw std::runtime_error(
            ...
            "\nAvailable commands:\n"
            "  vote-many   - Vote on a finalized budget (only locally with the active masternode)\n"
    ...
    if (strCommand == "vote-many" || strCommand == "vote") {
        ...
        return (strCommand == "vote-many" ? mnBudgetVoteInner(pwallet, hash, true, CBudgetVote::VOTE_YES, nullopt)
                                          : mnLocalBudgetVoteInner(hash));
    }
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ --------
    ...
    { "budget",             "getgovvotestatus",       &getgovvotestatus,       true,  {"proposal_hash"} },
    { "budget",             "getnextsuperblock",      &getnextsuperblock,      true,  {} },
    { "budget",             "listgovlocks",           &listgovlocks,           true,  {"proposal_hash"} },
    { "budget",             "castgovvote",            &castgovvote,            true,  {"proposal_hash","vote","lock_refs"} },
    { "budget",             "creategovvotelock",      &creategovvotelock,      true,  {"proposal_hash","amount","unlock_height","return_lock_ref"} },
    { "budget",             "mnbudgetrawvote",        &mnbudgetrawvote,        true,  {"collat_txid","collat_vout","hash","votecast","time","sig"} },
    { "budget",             "mnbudgetvote",           &mnbudgetvote,           true,  {"mode","hash","votecast","alias"} },
