...
UniValue getforkguardstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || !request.params.empty()) {
        throw std::runtime_error(
            "getforkguardstatus\n"
            "\nReturns divergence-recovery (ForkGuard) runtime status.\n"
            "\nResult:\n"
            "{\n"
            "  \"mode\": \"str\",                      (string) NORMAL|DIVERGED|RECOVERY|CAUTION\n"
            "  \"local_height\": n,                    (numeric) local tip height\n"
            "  \"peer_majority_height\": n,            (numeric) majority peer height sample\n"
            "  \"divergence_blocks\": n,               (numeric) absolute local/majority height delta\n"
            "  \"sampled_peers\": n,                   (numeric) peer count used for majority sample\n"
            "  \"sustained_samples\": n,               (numeric) current sustained divergence samples\n"
            "  \"required_sustained_samples\": n,      (numeric) samples required to enter recovery\n"
            "  \"staking_paused\": true|false,         (boolean) whether local staking is paused by ForkGuard\n"
            "  \"last_mode_change\": ttt,              (numeric) unix timestamp for last mode change\n"
            "  \"last_recovery_trigger\": ttt,         (numeric) unix timestamp for last recovery trigger\n"
            "  \"last_reason\": \"str\"                (string) reason attached to current/last mode transition\n"
            "}\n"
            "\nExamples:\n" +
            HelpExampleCli("getforkguardstatus", "") +
            HelpExampleRpc("getforkguardstatus", ""));
    }

    ForkGuardStatus status;
    if (!GetForkGuardStatus(status)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Unable to read ForkGuard status");
    }

    UniValue result(UniValue::VOBJ);
    result.pushKV("mode", status.mode);
    result.pushKV("local_height", status.local_height);
    result.pushKV("peer_majority_height", status.peer_majority_height);
    result.pushKV("divergence_blocks", status.divergence_blocks);
    result.pushKV("sampled_peers", status.sampled_peers);
    result.pushKV("sustained_samples", status.sustained_samples);
    result.pushKV("required_sustained_samples", status.required_sustained_samples);
    result.pushKV("staking_paused", status.staking_paused);
    result.pushKV("last_mode_change", status.last_mode_change);
    result.pushKV("last_recovery_trigger", status.last_recovery_trigger);
    result.pushKV("last_reason", status.last_reason);
    return result;
}
...
UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("addnode", "\"192.168.0.6:31616\" \"onetry\"") + HelpExampleRpc("addnode", "\"192.168.0.6:31616\", \"onetry\""));
...
UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:31616\",  (string) The pivx server IP and port we're connected to\n"
...
static UniValue addpeeraddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 2) {
        throw std::runtime_error(
            ...
            "\nExamples:\n"
            + HelpExampleCli("addpeeraddress", "\"1.2.3.4\" 31616")
            + HelpExampleRpc("addpeeraddress", "\"1.2.3.4\", 31616"));
    }
    if (!g_connman) {
        throw JSONRPCError(RPC_CLIENT_P2P_DISABLED, "Error: Peer-to-peer functionality missing or disabled");
    }
