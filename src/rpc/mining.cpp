...
UniValue setgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "In the Qt RPC console, append wallet=<name> to target a specific loaded wallet.\n"
            ...
            "\nUse the primary wallet in the Qt RPC console\n"
            "setgenerate true 1 wallet=\"\"\n"
            "\nUse a named wallet in the Qt RPC console\n"
            "setgenerate true 1 wallet=\"DeveloperWallet\"\n" +
            "\nCheck the setting\n" + HelpExampleCli("getgenerate", "") +
            "\nTurn off generation\n" + HelpExampleCli("setgenerate", "false") +
            "\nUsing json rpc\n" + HelpExampleRpc("setgenerate", "true, 1"));

    CWallet * const pwallet = GetWalletForJSONRPCRequest(request);
    EnsureWalletIsAvailable(pwallet, /*avoidException=*/false);
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "OrganicLife is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "OrganicLife is downloading blocks...");
...
//    result.pushKV("sizelimit", (int64_t)MAX_BLOCK_SIZE);
    ...
    result.pushKV("enforce_masternode_payments", GetMasternodePayment(pindexPrev->nHeight + 1) > 0);
