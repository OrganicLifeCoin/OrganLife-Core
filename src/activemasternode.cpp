...
OperationResult initMasternode(const std::string& _strMasterNodePrivKey, const std::string& _strMasterNodeAddr, bool isFromInit)
{
    ...
    if (fMasterNode && !isFromInit) {
        const bool hasActiveKey = activeMasternode.pubKeyMasternode.IsValid();
        const bool hasActiveAddr = activeMasternode.service.IsValid();

        if (hasActiveKey && activeMasternode.pubKeyMasternode != pubkey) {
            return errorOut("ERROR: Masternode already initialized with a different key.");
        }
        if (hasActiveAddr && activeMasternode.service != addrTest) {
            return errorOut("ERROR: Masternode already initialized with a different address.");
        }
    }
...
//
// Bootup the Masternode, look for a 10000 CTEAM input and register on the network
//
...
bool CActiveMasternode::SendMasternodePing(std::string& errorMessage)
{
    ...
    if (pmn != nullptr) {
        ...
        pmn->SetLastPing(mnp);
        mnodeman.AddSeenMasternodePing(mnp.GetHash(), mnp);

        CMasternodeBroadcast mnb(*pmn);
        mnodeman.UpdateSeenMasternodeBroadcastPing(mnb.GetHash(), mnp);
