...
static constexpr int64_t GOVERNANCE_COIN_WEIGHT_SCALE = 100000000;
static constexpr int64_t GOVERNANCE_COIN_WEIGHT_DEFAULT = GOVERNANCE_COIN_WEIGHT_SCALE / 100; // k=0.01
...
private:
    ...
    std::string strMasterPrivKey GUARDED_BY(cs);
    std::map<SporkId, CSporkDef*> sporkDefsById;
    std::map<std::string, CSporkDef*> sporkDefsByName;
    std::map<SporkId, CSporkMessage> mapSporksActive GUARDED_BY(cs);
