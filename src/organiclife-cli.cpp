...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    // Check for -testnet parameter (BaseParams() calls are only valid after this clause)
    try {
        ...
        do {
            try {
                ...
                if (!error.isNull()) {
                    ...
                    // Result
                    if (result.isNull()) {
                        if (strMethod == "setgenerate") {
                            bool generate = false;
                            int genProcLimit = -1;

                            if (params.isArray()) {
                                if (params.size() > 0 && params[0].isBool()) {
                                    generate = params[0].get_bool();
                                }
                                if (params.size() > 1 && params[1].isNum()) {
                                    genProcLimit = params[1].get_int();
                                }
                            } else if (params.isObject()) {
                                const UniValue genVal = find_value(params, "generate");
                                if (!genVal.isNull() && genVal.isBool()) {
                                    generate = genVal.get_bool();
                                }
                                const UniValue procVal = find_value(params, "genproclimit");
                                if (!procVal.isNull() && procVal.isNum()) {
                                    genProcLimit = procVal.get_int();
                                }
                            }

                            if (!generate || genProcLimit == 0) {
                                strPrint = "Generation disabled";
                            } else {
                                ...
                                if (cores <= 0) cores = 1;
                                strPrint = strprintf("Generation enabled on %d CPU core%s", cores, cores == 1 ? "" : "s");
                            }
                        } else {
                            strPrint = "";
                        }
