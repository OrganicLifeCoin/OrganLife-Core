...
std::set<NodeId> TierTwoConnMan::getQuorumNodes(Consensus::LLMQType llmqType, uint256 quorumHash)
{
    ...
    for (CNode* pnode : vNodesCopy) {
        ...
        if (!relayMembers.count(WITH_LOCK(pnode->cs_mnauth, return pnode->verifiedProRegTxHash))) {
            continue;
        }
...
void TierTwoConnMan::setMasternodeQuorumRelayMembers(Consensus::LLMQType llmqType, const uint256& quorumHash, const std::set<uint256>& proTxHashes)
{
    ...
    std::vector<CNode*> vNodesCopy = connman->CopyNodeVector();
    for (CNode* pnode : vNodesCopy) {
        if (!pnode->m_masternode_iqr_connection && isMasternodeQuorumRelayMember(WITH_LOCK(pnode->cs_mnauth, return pnode->verifiedProRegTxHash))) {
...
bool TierTwoConnMan::isMasternodeQuorumNode(const CNode* pnode, const CDeterministicMNList& mnList)
{
    ...
    uint256 verifiedProRegTxHash = WITH_LOCK(pnode->cs_mnauth, return pnode->verifiedProRegTxHash);
    if (verifiedProRegTxHash.IsNull() && !pnode->fInbound) {
    ...
    for (const auto& quorumConn : masternodeQuorumNodes) {
        if (!verifiedProRegTxHash.IsNull()) {
            if (quorumConn.second.count(verifiedProRegTxHash)) {
                return true;
            }
...
void TierTwoConnMan::ThreadOpenMasternodeConnections()
{
    ...
    while (!interruptNet) {
        ...
        for (CNode* pnode : vNodesCopy) {
            ...
            uint256 verifiedProRegTxHash = WITH_LOCK(pnode->cs_mnauth, return pnode->verifiedProRegTxHash);
            if (!verifiedProRegTxHash.IsNull()) {
                connectedMnServices.emplace_back(MnService{verifiedProRegTxHash, pnode->fInbound});
            }
        }
        connman->ReleaseNodeVector(vNodesCopy);
...
static void ProcessMasternodeConnections(CConnman& connman, TierTwoConnMan& tierTwoConnMan)
{
    ...
    std::vector<CNode*> vNodesCopy1 = connman.CopyNodeVector();
    for (CNode* pnode : vNodesCopy1) {
    ...
    if (nonMasternodeCount < (int) connman.GetMaxOutboundNodeCount()) {
        connman.ReleaseNodeVector(vNodesCopy1);
        return;
    }
    ...
    for (CNode* pnode : vNodesCopy1) {
        ...
        if (tierTwoConnMan.isMasternodeQuorumNode(pnode, mnList)) continue;
        // we're not disconnecting masternode probes for at least a few seconds
        if (pnode->m_masternode_probe_connection && GetSystemTimeInSeconds() - pnode->nTimeConnected < 5) continue;
