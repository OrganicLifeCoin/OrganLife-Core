...
private:
    ...
    unsigned int itemsFilterSize GUARDED_BY(cs_mapFulfilledRequests){0};
    unsigned int itemsFilterCount GUARDED_BY(cs_mapFulfilledRequests){0};
    int64_t filterCleanupTime GUARDED_BY(cs_mapFulfilledRequests){DEFAULT_ITEMS_FILTER_CLEANUP}; // for now, fixed cleanup time
    int64_t lastFilterCleanup GUARDED_BY(cs_mapFulfilledRequests){0};
