...
static int GovernanceHistoryRetentionBlocks()
{
    ...
    const int targetSpacing = std::max<int64_t>(1, consensus.nTargetSpacing);
    const int weekBlocks = static_cast<int>((7 * 24 * 60 * 60) / targetSpacing);
    const bool isMainnet = !Params().IsTestnet() && !Params().IsRegTestNet();
    const int cycleMultiplier = isMainnet ? 1 : 2;
    return std::max(consensus.nBudgetCycleBlocks * cycleMultiplier, weekBlocks);
}

static bool IsProposalInHistoryWindow(const CBudgetProposal& proposal, int nCurrentHeight)
{
    return proposal.IsExpired(nCurrentHeight) &&
           nCurrentHeight < proposal.GetBlockEnd() + GovernanceHistoryRetentionBlocks();
}

template <typename T>
static bool ShouldRelayBudgetItem(const T& item, int /*nCurrentHeight*/)
{
    return item.IsValid();
}

static bool ShouldRelayBudgetItem(const CBudgetProposal& proposal, int nCurrentHeight)
{
    return proposal.IsValid() || IsProposalInHistoryWindow(proposal, nCurrentHeight);
}

static constexpr int64_t MAX_GOVERNANCE_COIN_WEIGHT_FIXED = CBudgetProposal::HYBRID_SCORE_SCALE * 1000;

static int64_t SaturatingAddI64(int64_t current, int64_t delta)
{
    if (delta <= 0) {
        return current;
    }
...
static void ApplyCoinVoteTotalsFromIndex(CBudgetProposal& proposal)
{
    int64_t coinYeas = 0;
    int64_t coinNays = 0;
    if (governanceVoteIndex) {
        governanceVoteIndex->GetProposalTally(proposal.GetHash(), coinYeas, coinNays);
    }
    proposal.SetCoinVoteTotals(coinYeas, coinNays);
}

int64_t CBudgetManager::GetCoinGovernanceWeightFixed() const
{
    const int64_t sporkValue = sporkManager.GetSporkValue(SPORK_24_GOVERNANCE_COIN_WEIGHT);
    if (sporkValue < 0 || sporkValue > MAX_GOVERNANCE_COIN_WEIGHT_FIXED) {
        return DEFAULT_GOVERNANCE_COIN_WEIGHT_FIXED;
    }
    return sporkValue;
}
...
uint256 CBudgetManager::SubmitFinalBudget()
{
    ...
    if (!WITH_LOCK(cs_budgets, return mapUnconfirmedFeeTx.count(budgetHash))) {
        ...
        if (res.status == CWallet::CommitStatus::OK) {
            ...
            WITH_LOCK(cs_budgets, mapUnconfirmedFeeTx.emplace(budgetHash, collateraltxid));
    ...
    const uint256 collateralTxId = WITH_LOCK(cs_budgets, return mapUnconfirmedFeeTx.at(budgetHash));
    CFinalizedBudget fb(strBudgetName, nBlockStart, vecTxBudgetPayments, collateralTxId);
...
bool CBudgetManager::AddProposal(CBudgetProposal& budgetProposal)
{
    ...
    // Keep coin tallies aligned with indexed state before hybrid validity checks.
    ...
    if (!budgetProposal.UpdateValid(nCurrentHeight, mnCount)) {
        ...
        LogPrint(BCLog::MNBUDGET,
                "%s: accepting expired proposal %s for history sync (%s)\n",
                __func__,
                nHash.ToString(),
                budgetProposal.IsInvalidLogStr());
    }
...
{
    int nCurrentHeight = GetBestHeight();
    const int historyRetentionBlocks = GovernanceHistoryRetentionBlocks();
        ...
        for (auto& it: mapProposals) {
            ...
            if (!pbudgetProposal->UpdateValid(nCurrentHeight, mnCount)) {
                ...
                if (pbudgetProposal->IsExpired(nCurrentHeight) &&
                    ...
                    LogPrint(BCLog::MNBUDGET, "%s: Keeping expired proposal %s for history\n", __func__, pbudgetProposal->GetName());
    ...
    // Then check finalized budgets
    {
        ...
        for (auto& it: mapFinalizedBudgets) {
            ...
            if (!pfinalizedBudget->UpdateValid(nCurrentHeight)) {
                ...
                if (nCurrentHeight > pfinalizedBudget->GetBlockEnd() &&
                    nCurrentHeight < pfinalizedBudget->GetBlockEnd() + historyRetentionBlocks) {
                    LogPrint(BCLog::MNBUDGET, "%s: Keeping finalized budget %s for history\n", __func__, pfinalizedBudget->GetName());
...
bool CBudgetManager::FillBlockPayee(CMutableTransaction& txCoinbase, CMutableTransaction& txCoinstake, const int nHeight, bool fProofOfStake) const
{
    ...
    // Starting from CTEAM v6.0 masternode and budgets are paid in the coinbase tx of PoS blocks
...
bool CBudgetManager::GetProposal(const uint256& nHash, CBudgetProposal& bp) const
{
    ...
    ApplyCoinVoteTotalsFromIndex(bp);
    return true;
}

bool CBudgetManager::GetCoinVoteTotals(const uint256& proposalHash, int64_t& coinYeas, int64_t& coinNays) const
{
    coinYeas = 0;
    coinNays = 0;
    if (!governanceVoteIndex) {
    ...
    return governanceVoteIndex->GetProposalTally(proposalHash, coinYeas, coinNays);
}
...
std::vector<CBudgetProposal*> CBudgetManager::GetAllProposalsOrdered()
{
    ...
    std::sort(vBudgetProposalRet.begin(), vBudgetProposalRet.end(), [coinWeightFixed](CBudgetProposal* a, CBudgetProposal* b) {
        const int64_t scoreA = a->GetCombinedScore(coinWeightFixed);
        const int64_t scoreB = b->GetCombinedScore(coinWeightFixed);
        if (scoreA == scoreB) {
            return UintToArith256(a->GetFeeTXHash()) > UintToArith256(b->GetFeeTXHash());
        }
...
std::vector<CBudgetProposal> CBudgetManager::GetBudget()
{
    ...
    for (CBudgetProposal* pbudgetProposal: vProposalsOrdered) {
        ...
        if (pbudgetProposal->IsPassing(nBlockStart, nBlockEnd, mnCount, coinWeightFixed)) {
...
static bool relayItemIfFound(const uint256& itemHash, CNode* pfrom, RecursiveMutex& cs, std::map<uint256, T>& map, const char* type)
{
    ...
    const int nCurrentHeight = g_budgetman.GetBestHeight();
    LOCK(cs);
    const auto& it = map.find(itemHash);
    if (it == map.end()) return false;
    T* item = &(it->second);
    if (!ShouldRelayBudgetItem(*item, nCurrentHeight)) return true;
...
static void relayInventoryItems(CNode* pfrom, RecursiveMutex& cs, std::map<uint256, T>& map, bool fPartial, GetDataMsg invType, const int mn_sync_budget_type)
{
    ...
    int nInvCount = 0;
    {
        ...
        for (auto& it: map) {
            T* item = &(it.second);
            if (item && ShouldRelayBudgetItem(*item, nCurrentHeight)) {
...
void CBudgetManager::UpdateProposalCoinVotes(const uint256& proposalHash, int64_t coinAmount, bool fYes)
{
    if (coinAmount <= 0) return;

    LOCK(cs_proposals);
    auto it = mapProposals.find(proposalHash);
    if (it != mapProposals.end()) {
        if (fYes) {
            it->second.nCoinYeas = SaturatingAddI64(it->second.nCoinYeas, coinAmount);
        } else {
            it->second.nCoinNays = SaturatingAddI64(it->second.nCoinNays, coinAmount);
        }
    }
}

void CBudgetManager::DecrementProposalCoinVotes(const uint256& proposalHash, int64_t coinAmount, bool fYes)
{
    if (coinAmount <= 0) return;
    ...
    if (it != mapProposals.end()) {
        if (fYes) {
            it->second.nCoinYeas = std::max<int64_t>(0, it->second.nCoinYeas - coinAmount);
        } else {
            it->second.nCoinNays = std::max<int64_t>(0, it->second.nCoinNays - coinAmount);
        }
    }
}
