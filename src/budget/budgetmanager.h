...
protected:
    ...
    std::map<uint256, uint256> mapUnconfirmedFeeTx GUARDED_BY(cs_budgets);

    // map CollTx hash --> budget hash
    // keep track of collaterals for valid budgets/proposals (for reorgs)
    std::map<uint256, uint256> mapFeeTxToProposal GUARDED_BY(cs_proposals);
    std::map<uint256, uint256> mapFeeTxToBudget GUARDED_BY(cs_budgets);

    std::map<uint256, CBudgetProposal> mapProposals GUARDED_BY(cs_proposals);
    std::map<uint256, CFinalizedBudget> mapFinalizedBudgets GUARDED_BY(cs_budgets);

    std::map<uint256, CBudgetVote> mapSeenProposalVotes GUARDED_BY(cs_votes);
    typedef std::pair<std::vector<CBudgetVote>, int64_t> PropVotesAndLastVoteReceivedTime;
    std::map<uint256, PropVotesAndLastVoteReceivedTime> mapOrphanProposalVotes GUARDED_BY(cs_votes);
    std::map<uint256, CFinalizedBudgetVote> mapSeenFinalizedBudgetVotes GUARDED_BY(cs_finalizedvotes);
    typedef std::pair<std::vector<CFinalizedBudgetVote>, int64_t> BudVotesAndLastVoteReceivedTime;
    std::map<uint256, BudVotesAndLastVoteReceivedTime> mapOrphanFinalizedBudgetVotes GUARDED_BY(cs_finalizedvotes);
...
public:
    static constexpr int64_t DEFAULT_GOVERNANCE_COIN_WEIGHT_FIXED = CBudgetProposal::HYBRID_SCORE_SCALE / 100; // 0.01

    // critical sections to protect the inner data structures (must be locked in this order)
    mutable RecursiveMutex cs_budgets ACQUIRED_BEFORE(cs_proposals, cs_finalizedvotes, cs_votes);
    mutable RecursiveMutex cs_proposals ACQUIRED_BEFORE(cs_finalizedvotes, cs_votes);
    mutable RecursiveMutex cs_finalizedvotes ACQUIRED_BEFORE(cs_votes);
    ...
    CBudgetProposal* FindProposal(const uint256& nHash) EXCLUSIVE_LOCKS_REQUIRED(cs_proposals);
    CFinalizedBudget* FindFinalizedBudget(const uint256& nHash) EXCLUSIVE_LOCKS_REQUIRED(cs_budgets);
    ...
    bool GetCoinVoteTotals(const uint256& proposalHash, int64_t& coinYeas, int64_t& coinNays) const;
    void UpdateProposalCoinVotes(const uint256& proposalHash, int64_t coinAmount, bool fYes);
    void DecrementProposalCoinVotes(const uint256& proposalHash, int64_t coinAmount, bool fYes);
    ...
    int64_t GetCoinGovernanceWeightFixed() const;
    ...
    bool FillBlockPayee(CMutableTransaction& txCoinbase, CMutableTransaction& txCoinstake, int nHeight,
