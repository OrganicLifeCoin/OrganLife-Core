...
BOOST_AUTO_TEST_CASE(util_autogen_conf_defaults_to_mainnet_and_staking)
{
    ...
    const fs::path conf_path = GetConfigFile(PIVX_CONF_FILENAME);
    if (fs::exists(conf_path)) {
        fs::remove(conf_path);
    }

    gArgs.ReadConfigFile(PIVX_CONF_FILENAME);

    BOOST_REQUIRE(fs::exists(conf_path));

    std::ifstream conf_stream(conf_path.string(), std::ios::in | std::ios::binary);
    BOOST_REQUIRE(conf_stream.is_open());
    const std::string conf_content((std::istreambuf_iterator<char>(conf_stream)),
