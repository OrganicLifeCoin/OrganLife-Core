...
UniValue CallRPC(std::string args)
{
    ...
    try {
        ...
        if (objError.isObject()) {
            ...
            if (msg.isStr()) {
                throw std::runtime_error(msg.get_str());
            }
        }
        throw std::runtime_error(objError.write());
    }
...
BOOST_AUTO_TEST_CASE(rpc_convert_creategovvotelock_verbose_flag)
{
    ...
    const UniValue params = RPCConvertValues("creategovvotelock", args);
    BOOST_CHECK_EQUAL(params.size(), 4);
    BOOST_CHECK(params[3].isBool());
