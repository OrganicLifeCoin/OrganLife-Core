...
int main()
{
    ...
    assert(!pqwallet::IsPrivateWindowsConfigFile(configFile, config + "\n"));
    SetACL(configFile, privateACL + L"(A;;FR;;;WD)");
    assert(!pqwallet::IsPrivateWindowsConfigFile(configFile, config));
    SetACL(configFile, privateACL);
    const auto configAlias = directory + L"\\config-alias";
    assert(CreateHardLinkW(configAlias.c_str(), configFile.c_str(), nullptr));
    assert(!pqwallet::IsPrivateWindowsConfigFile(configFile, config));
    assert(DeleteFileW(configAlias.c_str()));
    assert(DeleteFileW(configFile.c_str()));
    std::cout << "Native Windows private inline-config checks passed\n";
