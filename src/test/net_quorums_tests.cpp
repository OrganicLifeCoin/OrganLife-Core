...
BOOST_FIXTURE_TEST_CASE(dkg_handler_preserves_session_when_init_fails, RegTestingSetup)
{
    auto& dkgManager = *llmq::quorumDKGSessionManager;
    auto& handler = dkgManager.dkgSessionHandlers.at(Consensus::LLMQ_TEST);

    auto oldSession = std::make_shared<llmq::CDKGSession>(handler.params, dkgManager.blsWorker, dkgManager);
    handler.curSession = oldSession;

    auto* savedActiveMasternodeManager = activeMasternodeManager;
    activeMasternodeManager = nullptr;

    const CBlockIndex* pindexQuorum = chainActive.Tip();
    BOOST_REQUIRE(pindexQuorum != nullptr);
    BOOST_CHECK(!handler.InitNewQuorum(pindexQuorum));
    BOOST_CHECK(handler.curSession == oldSession);

    activeMasternodeManager = savedActiveMasternodeManager;
}
