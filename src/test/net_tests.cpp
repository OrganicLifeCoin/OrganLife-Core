...
static uint256 AskForStressHash(uint32_t worker, uint32_t iter)
{
...
static void ClearAskedForForTest()
{
    LOCK(cs_mapAlreadyAskedFor);
    while (!mapAlreadyAskedFor.empty()) {
        mapAlreadyAskedFor.erase(mapAlreadyAskedFor.begin()->first);
    }
}
...
BOOST_AUTO_TEST_CASE(cnode_askfor_thread_safety_stress)
{
    ClearAskedForForTest();

    SOCKET hSocket = INVALID_SOCKET;
    in_addr ipv4Addr;
    ipv4Addr.s_addr = 0xa0b0c001;
    CAddress addr = CAddress(CService(ipv4Addr, 7777), NODE_NETWORK);
    CNode node(/*idIn=*/100, NODE_NETWORK, /*height=*/0, hSocket, addr, 0, 0, "", false);
...

    {
        ...
        for (const auto& item : node.mapAskFor) {
            BOOST_CHECK_EQUAL(node.setAskFor.count(item.second.hash), 1U);
        }
    }
...
BOOST_AUTO_TEST_CASE(connman_thread_stall_event)
{
    static constexpr int64_t kNowMicros = 120 * 1000000;
    static constexpr int64_t kTimeoutMicros = 30 * 1000000;

    BOOST_CHECK(ConnmanThreadGetStallEvent(kNowMicros, 0, kTimeoutMicros, false) == ConnmanThreadStallEvent::NONE);
    BOOST_CHECK(ConnmanThreadGetStallEvent(kNowMicros, kNowMicros - 10 * 1000000, kTimeoutMicros, false) == ConnmanThreadStallEvent::NONE);
    BOOST_CHECK(ConnmanThreadGetStallEvent(kNowMicros, kNowMicros - 30 * 1000000, kTimeoutMicros, false) == ConnmanThreadStallEvent::STALLED);
    BOOST_CHECK(ConnmanThreadGetStallEvent(kNowMicros, kNowMicros - 60 * 1000000, kTimeoutMicros, true) == ConnmanThreadStallEvent::NONE);
    BOOST_CHECK(ConnmanThreadGetStallEvent(kNowMicros, kNowMicros - 5 * 1000000, kTimeoutMicros, true) == ConnmanThreadStallEvent::RECOVERED);
}

BOOST_AUTO_TEST_CASE(connman_thread_auto_recover)
{
    static constexpr int64_t kNowMicros = 300 * 1000000;
    static constexpr int64_t kRecoverTimeoutMicros = 120 * 1000000;
    static constexpr int64_t kRecoverCooldownMicros = 60 * 1000000;

    BOOST_CHECK(!ConnmanThreadShouldAutoRecover(kNowMicros, 0, kRecoverTimeoutMicros, 0, kRecoverCooldownMicros));
    BOOST_CHECK(!ConnmanThreadShouldAutoRecover(kNowMicros, kNowMicros - 30 * 1000000, kRecoverTimeoutMicros, 0, kRecoverCooldownMicros));
    BOOST_CHECK(ConnmanThreadShouldAutoRecover(kNowMicros, kNowMicros - 130 * 1000000, kRecoverTimeoutMicros, 0, kRecoverCooldownMicros));
    BOOST_CHECK(!ConnmanThreadShouldAutoRecover(kNowMicros, kNowMicros - 130 * 1000000, kRecoverTimeoutMicros, kNowMicros - 10 * 1000000, kRecoverCooldownMicros));
    BOOST_CHECK(ConnmanThreadShouldAutoRecover(kNowMicros, kNowMicros - 130 * 1000000, kRecoverTimeoutMicros, kNowMicros - 80 * 1000000, kRecoverCooldownMicros));
}
