...
private:
    ...
    // PoW-only phase: tier-two objects are not expected to be available yet.
    // Keep the UI from getting stuck in an endless "syncing masternodes" loop and start
    // the real tier-two sync only once PoS is active.
    bool m_forcedSyncedUntilPoS{false};
