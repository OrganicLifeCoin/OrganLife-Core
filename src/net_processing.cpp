...
enum class InvalidBlockClass {
    FORK_SENSITIVE,
    MALICIOUS
};

struct ForkGuardState {
    ForkGuardMode mode{ForkGuardMode::NORMAL};
    int localHeight{-1};
    int peerMajorityHeight{-1};
    int divergenceBlocks{0};
    int sampledPeers{0};
    int sustainedSamples{0};
    int requiredSustainedSamples{0};
    int recoverySamples{0};
    int stableSamples{0};
    int64_t lastModeChange{0};
    int64_t lastRecoveryTrigger{0};
    std::string lastReason;
    bool stakingPaused{false};
};

ForkGuardState g_forkGuardState;
std::map<std::string, std::deque<int64_t>> g_forkSensitiveInvalidEventsByAddr;
static constexpr int64_t FORK_SENSITIVE_WINDOW_SECS = 20 * 60;
static constexpr int FORK_SENSITIVE_GRACE_EVENTS = 3;
static constexpr int FORK_SENSITIVE_REPEAT_PENALTY = 10;
static constexpr size_t FORK_SENSITIVE_MAX_EVENTS_PER_ADDR = 64;
static constexpr size_t FORK_SENSITIVE_MAX_TRACKED_ADDRS = 4096;

int64_t GetForkGuardTargetSpacing()
{
    return std::max<int64_t>(1, Params().GetConsensus().nTargetSpacing);
}

int ForkGuardBlocksForSeconds(int64_t seconds)
{
    return std::max(1, static_cast<int>(seconds / GetForkGuardTargetSpacing()));
}

int ForkGuardDivergenceThresholdBlocks()
{
    // Keep warning/recovery behavior proportional to chain timing across networks.
    const int defaultValue = std::max(3, ForkGuardBlocksForSeconds(600)); // 10 minutes
    return std::max(1, static_cast<int>(gArgs.GetArg("-forkguarddivergenceblocks", defaultValue)));
}

int ForkGuardSustainedSamplesThreshold()
{
    const int defaultValue = std::max(2, ForkGuardBlocksForSeconds(900)); // 15 minutes
    return std::max(1, static_cast<int>(gArgs.GetArg("-forkguardsustainedsamples", defaultValue)));
}

int ForkGuardRecoveryTimeoutSamples()
{
    const int defaultValue = std::max(4, ForkGuardBlocksForSeconds(1800)); // 30 minutes
    return std::max(1, static_cast<int>(gArgs.GetArg("-forkguardrecoverytimeoutsamples", defaultValue)));
}

int ForkGuardStableRecoverySamples()
{
    const int defaultValue = std::max(2, ForkGuardBlocksForSeconds(180)); // 3 minutes
    return std::max(1, static_cast<int>(gArgs.GetArg("-forkguardstablesamples", defaultValue)));
}

int ForkGuardMinimumPeerEvidence()
{
    // Keep this conservative enough to avoid false positives on tiny peer sets.
    return std::max(1, static_cast<int>(gArgs.GetArg("-forkguardminpeerevidence", 3)));
}

const char* ForkGuardModeToString(ForkGuardMode mode)
{
    switch (mode) {
    case ForkGuardMode::NORMAL: return "NORMAL";
    case ForkGuardMode::DIVERGED: return "DIVERGED";
    case ForkGuardMode::RECOVERY: return "RECOVERY";
    case ForkGuardMode::CAUTION: return "CAUTION";
    }
    return "UNKNOWN";
}

void ForkGuardSetMode(ForkGuardMode mode, const std::string& reason)
{
    const bool modeChanged = g_forkGuardState.mode != mode;
    g_forkGuardState.mode = mode;
    g_forkGuardState.lastReason = reason;
    if (modeChanged) {
        g_forkGuardState.lastModeChange = GetTime();
        LogPrintf("ForkGuard: mode -> %s (%s)\n", ForkGuardModeToString(mode), reason);
    }
}

void ForkGuardSetStakingPaused(bool paused, const std::string& reason)
{
    if (g_forkGuardState.stakingPaused == paused) return;
    g_forkGuardState.stakingPaused = paused;
    SetForkRecoveryPauseStaking(paused, reason);
    LogPrintf("ForkGuard: staking %s (%s)\n", paused ? "paused" : "resumed", reason);
}

void PruneForkSensitiveEventCache(const int64_t now)
{
    for (auto it = g_forkSensitiveInvalidEventsByAddr.begin(); it != g_forkSensitiveInvalidEventsByAddr.end(); ) {
        auto& events = it->second;
        while (!events.empty() && now - events.front() > FORK_SENSITIVE_WINDOW_SECS) {
            events.pop_front();
        }
        while (events.size() > FORK_SENSITIVE_MAX_EVENTS_PER_ADDR) {
            events.pop_front();
        }
        if (events.empty()) {
            it = g_forkSensitiveInvalidEventsByAddr.erase(it);
        } else {
            ++it;
        }
    }
...
bool ForkGuardGetPeerMajorityHeight(int& majorityHeight, int& sampledPeers);
std::vector<NodeId> ForkGuardCollectStalePeerCandidates() EXCLUSIVE_LOCKS_REQUIRED(cs_main);
void ForkGuardDisconnectNodeIds(CConnman* connman, const std::vector<NodeId>& nodeIds);
void ForkGuardUpdate(const CBlockIndex* pindexNew, bool fInitialDownload, std::vector<NodeId>* staleNodeIdsOut) EXCLUSIVE_LOCKS_REQUIRED(cs_main);

InvalidBlockClass ClassifyInvalidBlock(const CValidationState& state)
{
    const std::string reject = state.GetRejectReason();
    const std::string debug = state.GetDebugMessage();
    const auto hasPattern = [&](const std::string& pattern) {
        return reject.find(pattern) != std::string::npos || debug.find(pattern) != std::string::npos;
    };

    if (hasPattern("proof of stake check failed") ||
        hasPattern("kernel hash check fails") ||
        hasPattern("CheckProofOfStake") ||
        hasPattern("GetOldStakeModifier") ||
        hasPattern("bad-prevblk")) {
        return InvalidBlockClass::FORK_SENSITIVE;
    }

    return InvalidBlockClass::MALICIOUS;
}
...
struct CNodeState {
    ...
    CNodeState(CAddress addrIn, std::string addrNameIn) : address(addrIn), name(addrNameIn) {
        ...
        nValidBlocksReceived = 0;
        nMessagesReceived = 0;
        fReliabilityScore = 1.0;
    }
};
...
bool ForkGuardGetPeerMajorityHeight(int& majorityHeight, int& sampledPeers)
{
    ...
    sampledPeers = 0;
    for (const auto& entry : mapNodeState) {
        const CNodeState& state = entry.second;
        int peerHeight = -1;
        if (state.pindexLastCommonBlock != nullptr) {
            peerHeight = std::max(peerHeight, state.pindexLastCommonBlock->nHeight);
        }
        if (state.pindexBestKnownBlock != nullptr) {
            peerHeight = std::max(peerHeight, state.pindexBestKnownBlock->nHeight);
        }
        if (peerHeight <= 0) {
            continue;
        }

        heightCounts[peerHeight]++;
        sampledPeers++;
    }

    if (heightCounts.empty()) {
        majorityHeight = chainActive.Height();
        return false;
    }

    int maxCount = 0;
    majorityHeight = chainActive.Height();
    for (const auto& item : heightCounts) {
        if (item.second > maxCount || (item.second == maxCount && item.first > majorityHeight)) {
            maxCount = item.second;
            majorityHeight = item.first;
        }
    }
    return true;
}

std::vector<NodeId> ForkGuardCollectStalePeerCandidates()
{
    std::vector<NodeId> candidates;
    candidates.reserve(8);
    for (const auto& entry : mapNodeState) {
        const CNodeState& state = entry.second;
        const bool lowReliability = state.fReliabilityScore < 0.25;
        const bool hasMisbehavior = state.nMisbehavior > 0;
        if (lowReliability || hasMisbehavior) {
...
void ForkGuardDisconnectNodeIds(CConnman* connman, const std::vector<NodeId>& nodeIds)
{
    if (!connman || nodeIds.empty()) return;

    int disconnected = 0;
    for (const NodeId& nodeId : nodeIds) {
        if (disconnected >= 4) break;
        connman->ForNode(nodeId, [&disconnected](CNode* pnode) {
            if (pnode->fInbound || pnode->fWhitelisted || pnode->fAddnode || pnode->fOneShot) return false;
    ...
    if (disconnected > 0) {
        LogPrintf("ForkGuard: disconnected %d stale outbound peer(s) to refresh sync sources\n", disconnected);
    }
}

void ForkGuardUpdate(const CBlockIndex* pindexNew, bool fInitialDownload, std::vector<NodeId>* staleNodeIdsOut) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    if (staleNodeIdsOut) staleNodeIdsOut->clear();

    if (fInitialDownload || pindexNew == nullptr) {
        return;
    }

    int majorityHeight = chainActive.Height();
    int sampledPeers = 0;
    const bool hasMajority = ForkGuardGetPeerMajorityHeight(majorityHeight, sampledPeers);
    const int localHeight = pindexNew->nHeight;
    const int divergence = std::abs(localHeight - majorityHeight);
    const int divergenceThreshold = ForkGuardDivergenceThresholdBlocks();
    const int sustainedRequired = ForkGuardSustainedSamplesThreshold();
    const int stableRequired = ForkGuardStableRecoverySamples();
    const int recoveryTimeout = ForkGuardRecoveryTimeoutSamples();
    const int minPeerEvidence = ForkGuardMinimumPeerEvidence();

    g_forkGuardState.localHeight = localHeight;
    g_forkGuardState.peerMajorityHeight = majorityHeight;
    g_forkGuardState.divergenceBlocks = divergence;
    g_forkGuardState.sampledPeers = sampledPeers;
    g_forkGuardState.requiredSustainedSamples = sustainedRequired;

    const int64_t catchUpLag = std::max<int64_t>(60, GetForkGuardTargetSpacing() * 2);
    const bool isCatchingUp = pindexNew->GetBlockTime() + catchUpLag < GetAdjustedTime();
    if (isCatchingUp) {
        g_forkGuardState.sustainedSamples = 0;
        g_forkGuardState.stableSamples = 0;
        g_forkGuardState.recoverySamples = 0;
        ForkGuardSetStakingPaused(false, "historical catch-up");
        ForkGuardSetMode(ForkGuardMode::CAUTION, "historical catch-up");
        return;
    }

    if (!hasMajority || sampledPeers < minPeerEvidence) {
        g_forkGuardState.sustainedSamples = 0;
        g_forkGuardState.stableSamples = 0;
        g_forkGuardState.recoverySamples = 0;
        ForkGuardSetStakingPaused(false, "insufficient peer evidence");
        ForkGuardSetMode(ForkGuardMode::CAUTION, "insufficient peer evidence");
        return;
    }

    if (divergence >= divergenceThreshold) {
        g_forkGuardState.sustainedSamples++;
        g_forkGuardState.stableSamples = 0;

        if (g_forkGuardState.mode != ForkGuardMode::RECOVERY) {
            ForkGuardSetMode(ForkGuardMode::DIVERGED, strprintf("divergence=%d majority=%d local=%d", divergence, majorityHeight, localHeight));
        }

        if (g_forkGuardState.sustainedSamples >= sustainedRequired) {
            if (g_forkGuardState.mode != ForkGuardMode::RECOVERY) {
                g_forkGuardState.recoverySamples = 0;
                g_forkGuardState.lastRecoveryTrigger = GetTime();
                ForkGuardSetMode(ForkGuardMode::RECOVERY, "sustained divergence confirmed");
                ForkGuardSetStakingPaused(true, "fork recovery active");
                if (staleNodeIdsOut) {
                    *staleNodeIdsOut = ForkGuardCollectStalePeerCandidates();
                }
            }
        }
    } else {
        g_forkGuardState.sustainedSamples = 0;
        if (g_forkGuardState.mode == ForkGuardMode::RECOVERY) {
            g_forkGuardState.stableSamples++;
            if (g_forkGuardState.stableSamples >= stableRequired) {
                ForkGuardSetStakingPaused(false, "chain convergence restored");
                ForkGuardSetMode(ForkGuardMode::NORMAL, "recovered");
                g_forkGuardState.recoverySamples = 0;
            }
        } else {
            g_forkGuardState.stableSamples = 0;
            ForkGuardSetMode(ForkGuardMode::NORMAL, "healthy");
        }
    }

    if (g_forkGuardState.mode == ForkGuardMode::RECOVERY) {
        g_forkGuardState.recoverySamples++;
        if (g_forkGuardState.recoverySamples >= recoveryTimeout) {
            ForkGuardSetStakingPaused(false, "recovery timeout reached");
            ForkGuardSetMode(ForkGuardMode::CAUTION, "recovery timeout reached");
            g_forkGuardState.sustainedSamples = 0;
            g_forkGuardState.recoverySamples = 0;
        }
    }
...
bool GetNodeStateStats(NodeId nodeid, CNodeStateStats& stats)
{
    ...
    stats.nValidBlocksReceived = state->nValidBlocksReceived;
    stats.nMessagesReceived = state->nMessagesReceived;
...
{
    LOCK(cs_main);
    status.mode = ForkGuardModeToString(g_forkGuardState.mode);
    status.local_height = g_forkGuardState.localHeight;
    status.peer_majority_height = g_forkGuardState.peerMajorityHeight;
    status.divergence_blocks = g_forkGuardState.divergenceBlocks;
    status.sampled_peers = g_forkGuardState.sampledPeers;
    status.sustained_samples = g_forkGuardState.sustainedSamples;
    status.required_sustained_samples = g_forkGuardState.requiredSustainedSamples;
    status.staking_paused = g_forkGuardState.stakingPaused;
    status.last_mode_change = g_forkGuardState.lastModeChange;
    status.last_recovery_trigger = g_forkGuardState.lastRecoveryTrigger;
    status.last_reason = g_forkGuardState.lastReason;
    return true;
}
...
void Misbehaving(NodeId pnode, int howmuch, const std::string& message) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    int banscore = gArgs.GetArg("-banscore", GetDynamicBanScoreThreshold());
...
void HandleInvalidBlockFromPeer(NodeId nodeid, const CValidationState& state, int nDoS, CConnman* connman) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (ClassifyInvalidBlock(state) != InvalidBlockClass::FORK_SENSITIVE) {
        if (nDoS > 0) {
            Misbehaving(nodeid, nDoS, state.GetRejectReason());
        }
        UpdatePeerReliability(nodeid, false);
        return;
    }

    const int64_t now = GetTime();
    PruneForkSensitiveEventCache(now);
    const std::string peerKey = peerState->address.ToStringIP();
    auto eventsIt = g_forkSensitiveInvalidEventsByAddr.find(peerKey);
    if (eventsIt == g_forkSensitiveInvalidEventsByAddr.end() &&
        g_forkSensitiveInvalidEventsByAddr.size() < FORK_SENSITIVE_MAX_TRACKED_ADDRS) {
        eventsIt = g_forkSensitiveInvalidEventsByAddr.emplace(peerKey, std::deque<int64_t>{}).first;
    }
    std::deque<int64_t> untrackedEvents;
    std::deque<int64_t>& events = eventsIt != g_forkSensitiveInvalidEventsByAddr.end() ? eventsIt->second : untrackedEvents;
    ...
    const std::string rejectReason = state.GetRejectReason().empty() ? "fork-sensitive invalid PoS block" : state.GetRejectReason();

    if (eventCount <= FORK_SENSITIVE_GRACE_EVENTS) {
        LogPrintf("ForkGuard: peer=%d addr=%s fork-sensitive invalid block event %d/%d in %d sec, disconnecting without ban (%s)\n",
                  nodeid, peerKey, eventCount, FORK_SENSITIVE_GRACE_EVENTS, FORK_SENSITIVE_WINDOW_SECS, rejectReason);
    } else {
        LogPrintf("ForkGuard: peer=%d addr=%s fork-sensitive invalid block event %d in %d sec, staged penalty=%d (%s)\n",
                  nodeid, peerKey, eventCount, FORK_SENSITIVE_WINDOW_SECS, FORK_SENSITIVE_REPEAT_PENALTY, rejectReason);
        Misbehaving(nodeid, FORK_SENSITIVE_REPEAT_PENALTY, "fork-sensitive invalid PoS block");
    }
...
void WarningStaging(NodeId nodeid, int howmuch, const std::string& message) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    // For minor offenses, warn first (10 points), then ban if it repeats (20 points)
    int warningThreshold = gArgs.GetArg("-banscore", GetDynamicBanScoreThreshold()) * 0.3; // 30% threshold for warnings
    
    if (state->nMisbehavior < warningThreshold) {
        // First offense - just a warning
        state->nMisbehavior += std::min(howmuch, 10);
        std::string message_prefixed = message.empty() ? "" : (": " + message);
        LogPrint(BCLog::NET, "%s: WARNING %s peer=%d (%d)%s\n", __func__, state->name, nodeid, state->nMisbehavior, message_prefixed);
    } else {
        // Repeated offense - full penalty
        Misbehaving(nodeid, howmuch, message);
    }
}

// Requires cs_main.
void UpdatePeerReliability(NodeId nodeid, bool validData) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    CNodeState* state = State(nodeid);
    if (!state)
        return;

    state->nMessagesReceived++;
    if (validData) {
        state->nValidBlocksReceived++;
    }
    
    state->fReliabilityScore = std::clamp(
        (double)state->nValidBlocksReceived / std::max((double)state->nMessagesReceived, 1.0),
        0.0, 1.0
    );
}
...
double GetPeerReliabilityScore(NodeId nodeid) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    CNodeState* state = State(nodeid);
    if (!state)
        return 1.0; // Default to reliable for new connections
    return state->fReliabilityScore;
}
...
{
    std::vector<NodeId> staleNodeIds;
    {
        LOCK(cs_main);
        ForkGuardUpdate(pindexNew, fInitialDownload, &staleNodeIds);
    }

    if (!staleNodeIds.empty()) {
        ForkGuardDisconnectNodeIds(connman, staleNodeIds);
    }
...
void PeerLogicValidation::BlockChecked(const CBlock& block, const CValidationState& state)
{
    ...
    if (it != mapBlockSource.end() && State(it->second)) {
        if (state.IsInvalid(nDoS)) {
            ...
            HandleInvalidBlockFromPeer(it->second, state, nDoS, connman);
            // Spam filter
            CheckBlockSpam(it->second, block.GetHash());
        } else if (state.IsValid()) {
            UpdatePeerReliability(it->second, true);
        }
    }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived, CConnman* connman, std::atomic<bool>& interruptMsgProc)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // CTEAM: We use certain sporks during IBD, so check to see if they are
    ...
    else if (!pfrom->fSuccessfullyConnected)
    {
        ...
        if (pfrom->m_last_wants_recsigs_recv > 0 &&
            ...
            LOCK(cs_main);
            WarningStaging(pfrom->GetId(), 20, "sendrecssigs msg is only accepted every 20 minutes");
            return false;
        }
    ...
    if (strCommand == NetMsgType::ADDR || strCommand == NetMsgType::ADDRV2) {
        ...
        if (vAddr.size() > MAX_ADDR_TO_SEND) {
            LOCK(cs_main);
            WarningStaging(pfrom->GetId(), 20, strprintf("%s message size = %u", strCommand, vAddr.size()));
            return false;
        }
        ...
        if (vInv.size() > MAX_INV_SZ) {
            LOCK(cs_main);
            WarningStaging(pfrom->GetId(), 20, strprintf("message inv size() = %u", vInv.size()));
            return false;
        }
        ...
        if (vInv.size() > MAX_INV_SZ) {
            LOCK(cs_main);
            WarningStaging(pfrom->GetId(), 20, strprintf("message getdata size() = %u", vInv.size()));
            return false;
        }
        ...
        {
            LOCK2(cs_mapAlreadyAskedFor, pfrom->cs_inventory);
    ...
    else if (strCommand == NetMsgType::HEADERS && Params().HeadersFirstSyncingActive() && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        const bool fInitialDownload = IsInitialBlockDownload();

        // Bypass the normal CBlock deserialization, as we don't want to risk deserializing 2000 full blocks.
        unsigned int nCount = ReadCompactSize(vRecv);
        if (nCount > MAX_HEADERS_RESULTS) {
            LOCK(cs_main);
            WarningStaging(pfrom->GetId(), 20, strprintf("headers message size = %u", nCount));
            return false;
        }
        ...
        for (const CBlockHeader& header : headers) {
            ...
            if (pindexLast && header.hashPrevBlock != pindexLast->GetBlockHash()) {
                const int headerSequencePenalty = fInitialDownload ? 10 : 20;
                Misbehaving(pfrom->GetId(), headerSequencePenalty, "non-continuous headers sequence");
                return false;
            }
...
{
    {
        ...
        //
        std::vector<CInv> vPendingGetData;
        {
            LOCK(pto->cs_inventory);
            while (!pto->mapAskFor.empty() && (*pto->mapAskFor.begin()).first <= nNow) {
                vPendingGetData.emplace_back((*pto->mapAskFor.begin()).second);
                pto->mapAskFor.erase(pto->mapAskFor.begin());
            }
        }
        for (const CInv& inv : vPendingGetData) {
            if (!AlreadyHave(inv)) {
