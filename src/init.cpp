...
#ifdef ENABLE_WALLET
static RecursiveMutex cs_wallet_staking_threads;
static std::map<CWallet*, boost::thread*> walletStakingThreads;

CScheduler& GetNodeScheduler()
{
    return scheduler;
}

bool StartWalletStakingThread(CWallet* pwallet)
{
    if (!pwallet || !gArgs.GetBoolArg("-staking", !Params().IsRegTestNet() && DEFAULT_STAKING)) {
        return false;
    }

    LOCK(cs_wallet_staking_threads);
    if (walletStakingThreads.count(pwallet) != 0) {
        return false;
    }

    boost::thread* stakingThread = threadGroup.create_thread(std::bind(&ThreadStakeMinter, pwallet));
    walletStakingThreads.emplace(pwallet, stakingThread);
    return true;
}

void StopWalletStakingThread(CWallet* pwallet)
{
    boost::thread* stakingThread = nullptr;
    {
        LOCK(cs_wallet_staking_threads);
        const auto it = walletStakingThreads.find(pwallet);
        if (it == walletStakingThreads.end()) {
            return;
        }
        stakingThread = it->second;
        walletStakingThreads.erase(it);
    }
    ...
    stakingThread->interrupt();
    stakingThread->join();
    threadGroup.remove_thread(stakingThread);
    delete stakingThread;
}

void StopAllWalletStakingThreads()
{
    std::vector<CWallet*> wallets;
    {
        LOCK(cs_wallet_staking_threads);
        wallets.reserve(walletStakingThreads.size());
        for (const auto& [wallet, thread] : walletStakingThreads) {
...
static boost::signals2::connection g_rpcNotifyBlockTipConnection;

void OnRPCStarted()
{
    g_rpcNotifyBlockTipConnection = uiInterface.NotifyBlockTip.connect(RPCNotifyBlockChange);
}

void OnRPCStopped()
{
    g_rpcNotifyBlockTipConnection.disconnect();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-banscore=<n>", strprintf("Threshold for disconnecting misbehaving peers (default: %u, doubled during IBD)", DEFAULT_BANSCORE_THRESHOLD));
    ...
    if (showDebug) {
        ...
        strUsage += HelpMessageOpt("-forkguarddivergenceblocks=<n>", "Override ForkGuard divergence trigger in blocks (default: time-scaled by target spacing)");
        strUsage += HelpMessageOpt("-forkguardsustainedsamples=<n>", "Override ForkGuard sustained divergence samples required before recovery");
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The %s Developers"), COPYRIGHT_YEAR, PACKAGE_NAME)) + "\n" +
           "\n" +
...
bool AppInitParameterInteraction()
{
    ...
    if (gArgs.IsArgSet("-minrelaytxfee")) {
        ...
        // Keep regtest relay policy aligned with the functional test assumptions.
        // This affects only regtest when no explicit -minrelaytxfee is provided.
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                pcoinsdbview.reset();
                pcoinscatcher.reset();
                // IMPORTANT: Ensure the previous LevelDB handle is destroyed before we
        ...
        if (!fLoaded && !ShutdownRequested()) {
            if (!fReset) {
                ...
                return UIError(strLoadError + ".\n\n" +
                               _("Block database rebuild failed. Make sure no other instance is running and that you have enough disk space/permissions. "
                                 "If the problem persists, try restarting with -reindex, or start with a fresh -datadir."));
            }
        }
    }
...
// ********************************************************* Step 8: Backup and Load wallet
    ...
    boost::signals2::scoped_connection genesisBlockTipConnection;
    if (chainActive.Tip() == nullptr) {
        genesisBlockTipConnection = uiInterface.NotifyBlockTip.connect(BlockNotifyGenesisWait);
    } else {
        fHaveGenesis = true;
    }
    ...
    if (!vpwallets.empty() && gArgs.GetBoolArg("-staking", !Params().IsRegTestNet() && DEFAULT_STAKING)) {
        for (CWalletRef pwallet : vpwallets) {
            StartWalletStakingThread(pwallet);
        }
    }
#endif
