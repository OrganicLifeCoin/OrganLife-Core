...
int64_t UpdateTime(CBlockHeader* pblock, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // Time Protocol V2 requires timestamp to be aligned to time slots (15 seconds)
    const int nHeight = pindexPrev->nHeight + 1;
    if (consensusParams.IsTimeProtocolV2(nHeight)) {
        nNewTime = GetTimeSlot(nNewTime);
        // Ensure we don't create a block with the same timestamp as the parent
        if (nNewTime <= pindexPrev->GetBlockTime()) {
            nNewTime = pindexPrev->GetBlockTime() + consensusParams.nTimeSlotLength;
        }
    }
...
{
    (void)entry;
    // Sort package by ancestor count and then resolve any additional ordering
    // constraints that are not represented as vin dependencies (e.g. governance
    // vote casts referencing governance lock tx hashes).
    std::vector<CTxMemPool::txiter> pending;
    pending.insert(pending.end(), package.begin(), package.end());
    std::sort(pending.begin(), pending.end(), CompareTxIterByAncestorCount());
    ...
    auto hasPendingGovLockDependency = [&](const CTxMemPool::txiter& txit) {
        ...
        for (const COutPoint& lockRef : castPayload.lockRefs) {
            const auto lockIt = mempool.mapTx.find(lockRef.hash);
            if (lockIt == mempool.mapTx.end()) {
                continue;
            }
            if (package.count(lockIt) == 0) {
                continue;
            }
            if (included.count(lockRef.hash) == 0) {
                return true;
            }
        }
        return false;
    ...
    while (!pending.empty()) {
        ...
        for (auto it = pending.begin(); it != pending.end(); ++it) {
            ...
            for (const auto& parent : mempool.GetMemPoolParents(candidate)) {
                if (package.count(parent) != 0 && included.count(parent->GetTx().GetHash()) == 0) {
                    parentsReady = false;
                    break;
                }
            }
            if (!parentsReady || hasPendingGovLockDependency(candidate)) {
                continue;
            }

            sortedEntries.emplace_back(candidate);
            included.insert(candidate->GetTx().GetHash());
            pending.erase(it);
            progressed = true;
            break;
        }
        ...
        if (!progressed) {
            // Fallback to deterministic order if an unexpected cycle is found.
            sortedEntries.insert(sortedEntries.end(), pending.begin(), pending.end());
            break;
        }
    }
}
...
void BlockAssembler::addPackageTxs()
{
    ...
    while (mi != mempool.mapTx.get<ancestor_score>().end() || !mapModifiedTx.empty())
    {
        ...
        // Include non-vin governance dependencies (vote-cast -> vote-lock) when
        // referenced lock txes are still in mempool.
        bool addedGovDependencies = true;
        while (addedGovDependencies) {
            ...
            for (const auto& candidate : current) {
                ...
                for (const COutPoint& lockRef : castPayload.lockRefs) {
                    auto lockIt = mempool.mapTx.find(lockRef.hash);
                    if (lockIt == mempool.mapTx.end() || inBlock.count(lockIt) != 0) {
                        continue;
                    }
                    if (ancestors.count(lockIt) == 0) {
                        CTxMemPool::setEntries lockAncestors;
                        mempool.CalculateMemPoolAncestors(*lockIt, lockAncestors, nNoLimit, nNoLimit, nNoLimit, nNoLimit, dummy, false);
                        onlyUnconfirmed(lockAncestors);
                        ancestors.insert(lockAncestors.begin(), lockAncestors.end());
                        ancestors.insert(lockIt);
                        addedGovDependencies = true;
                    }
                }
            }
        }

        uint64_t packageSizeWithGovDeps = 0;
        CAmount packageFeesWithGovDeps = 0;
        unsigned int packageSigOpsWithGovDeps = 0;
        for (const auto& txIt : ancestors) {
            packageSizeWithGovDeps += txIt->GetTxSize();
            packageFeesWithGovDeps += txIt->GetModifiedFee();
            packageSigOpsWithGovDeps += txIt->GetSigOpCount();
        }

        if (packageFeesWithGovDeps < ::minRelayTxFee.GetFee(packageSizeWithGovDeps) ||
                !TestPackage(packageSizeWithGovDeps, packageSigOpsWithGovDeps)) {
            if (fUsingModified) {
                mapModifiedTx.get<ancestor_score>().erase(modit);
                failedTx.insert(iter);
            }
