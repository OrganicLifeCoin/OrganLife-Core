 ...
 *
 * Default min relay fee is 0.01 coin per kB.
 */
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (!Params().IsRegTestNet() && consensus.nPremineReward > 0) {
        ...
        // - Premine: chain-defined launch allocation at height 1
        // - Subsidy: fixed chain-defined block reward until cap is reached, then 0
        //
        // Note: The genesis coinbase is unspendable and is not part of the circulating supply.
        const CAmount premine = consensus.nPremineReward;
        const CAmount blockSubsidy = consensus.nBlockSubsidy;

        if (nHeight <= 0) return 0;
        if (nHeight == 1) return premine;
        if (blockSubsidy <= 0) return 0;

        const CAmount maxMoneyOut = consensus.nMaxMoneyOut;
        const int64_t rewardBlocksMined = nHeight - 2; // height 2 is the first "normal subsidy" block
        const CAmount alreadyMinted = premine + (rewardBlocksMined * blockSubsidy);
        const CAmount remaining = maxMoneyOut - alreadyMinted;
    ...
    // Legacy block reward reduction schedule.
    if (nHeight > nLast) return 10 * COIN;
    ...
    // Legacy dev/test premine kept for non-mainnet compatibility.
    return 60001 * COIN;
}
...
int GetDynamicBanScoreThreshold()
{
    return IsInitialBlockDownload() ?
           DEFAULT_BANSCORE_THRESHOLD_IBD :
           DEFAULT_BANSCORE_THRESHOLD;
}

static std::atomic<bool> g_forkRecoveryPauseStaking{false};

bool IsForkRecoveryPauseStaking()
{
    return g_forkRecoveryPauseStaking.load();
}

void SetForkRecoveryPauseStaking(bool pause, const std::string& reason)
{
    g_forkRecoveryPauseStaking.store(pause);
    (void)reason;
}
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    int nDefaultMaxReorg = DEFAULT_MAX_REORG_DEPTH;
    if (Params().IsTestnet()) {
        nDefaultMaxReorg = DEFAULT_MAX_REORG_DEPTH_TESTNET;
    } else {
        nDefaultMaxReorg = DEFAULT_MAX_REORG_DEPTH_MAINNET;
    }
    int nMaxReorgDepth = gArgs.GetArg("-maxreorg", nDefaultMaxReorg);
...
/*
 * Collect the set of the inputs (regular utxos) spent
 * by in-block txes.
 * Also, check that there are no in-block double spends.
 */
static bool CheckInBlockDoubleSpends(const CBlock& block, CValidationState& state,
...
static bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, const FlatFilePos* dbp) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (isPoS) {
        ...
        std::unordered_set<COutPoint, SaltedOutpointHasher> spent_outpoints;
        if (!CheckInBlockDoubleSpends(block, state, spent_outpoints)) {
            return false;
        }
...
bool ProcessNewBlock(const std::shared_ptr<const CBlock>& pblock, const FlatFilePos* dbp)
{
    ...
    try {
        ...
        if (nLockTime > 500) {
            LogPrintf("%s: WARNING: Took %ld ms to acquire cs_main lock for block %s\n", __func__, nLockTime, pblock->GetHash().ToString());
        }
        ...
        return error("%s: Exception during block processing: %s", __func__, e.what());
    }

    // Activating best chain also needs lock in many places
    int64_t nActivateStartTime = GetTime();
    int64_t nActivateEndTime = 0;
    ...
    if (nTotalTime > 10) {
        ...
        if (nTotalTime > 20) {
            LogPrintf("%s: CRITICAL: Block %s processing took extremely long (%ld seconds)\n", __func__, pblock->GetHash().ToString(), nTotalTime);
            LogPrintf("%s: This may indicate a deadlock or lock acquisition problem\n", __func__);
        }
    }
