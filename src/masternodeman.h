...
private:
    ...
    std::map<COutPoint, MasternodeRef> mapMasternodes GUARDED_BY(cs);
    // who's asked for the Masternode list and the last time
    std::map<CNetAddr, int64_t> mAskedUsForMasternodeList GUARDED_BY(cs);
    // who we asked for the Masternode list and the last time
    std::map<CNetAddr, int64_t> mWeAskedForMasternodeList GUARDED_BY(cs);
    // which Masternodes we've asked for
    std::map<COutPoint, int64_t> mWeAskedForMasternodeListEntry GUARDED_BY(cs);
    ...
    CyclingVector<uint256> cvLastBlockHashes GUARDED_BY(cs);

    // Return the banning score (0 if no ban score increase is needed).
    int ProcessMNBroadcast(CNode* pfrom, CMasternodeBroadcast& mnb) EXCLUSIVE_LOCKS_REQUIRED(cs_process_message);
    int ProcessMNPing(CNode* pfrom, CMasternodePing& mnp) EXCLUSIVE_LOCKS_REQUIRED(cs_process_message);
    int ProcessMessageInner(CNode* pfrom, std::string& strCommand, CDataStream& vRecv) EXCLUSIVE_LOCKS_REQUIRED(cs_process_message);

    // Relay a MN
    void BroadcastInvMN(CMasternode* mn, CNode* pfrom) EXCLUSIVE_LOCKS_REQUIRED(cs);
...
public:
    ...
    std::map<uint256, CMasternodeBroadcast> mapSeenMasternodeBroadcast GUARDED_BY(cs);
    // Keep track of all pings I've seen
    std::map<uint256, CMasternodePing> mapSeenMasternodePing GUARDED_BY(cs);
    ...
    RecursiveMutex& GetLock() const LOCK_RETURNED(cs) { return cs; }
    ...
    void Clear();
    void CleanupInvalidSeenBroadcasts();
    ...
    bool HasSeenMasternodeBroadcast(const uint256& hash) const;
    bool HasSeenMasternodePing(const uint256& hash) const;
    bool GetSeenMasternodeBroadcast(const uint256& hash, CMasternodeBroadcast& mnb) const;
    bool GetSeenMasternodePing(const uint256& hash, CMasternodePing& mnp) const;
    void AddSeenMasternodeBroadcast(const uint256& hash, const CMasternodeBroadcast& mnb);
    void AddSeenMasternodePing(const uint256& hash, const CMasternodePing& mnp);
    void RemoveSeenMasternodeBroadcast(const uint256& hash);
    void UpdateSeenMasternodeBroadcastPing(const uint256& hash, const CMasternodePing& ping);
    ...
    MasternodeRef GetCurrentMasterNode(const uint256& hash, bool onlyEnabled = true) const;
    ...
    std::vector<uint256> GetCachedBlocks() const EXCLUSIVE_LOCKS_REQUIRED(cs) { return cvLastBlockHashes.GetCache(); }
};
