...
public:
    int nBlockHeight;
    std::vector<CMasternodePayee> vecPayments GUARDED_BY(cs_vecPayments);
...
public:
    std::map<uint256, CMasternodePaymentWinner> mapMasternodePayeeVotes GUARDED_BY(cs_mapMasternodePayeeVotes);
    std::map<int, CMasternodeBlockPayees> mapMasternodeBlocks GUARDED_BY(cs_mapMasternodeBlocks);
    ...
    bool GetLegacyMasternodePayee(int nHeight, CScript& payee) const;
    ...
    bool HasMasternodeWinner(const uint256& winnerHash) const;
    bool GetMasternodeWinner(const uint256& winnerHash, CMasternodePaymentWinner& winnerRet) const;
    std::string GetRequiredPaymentsString(int nBlockHeight);
    ...
    SERIALIZE_METHODS(CMasternodePayments, obj) {
        LOCK2(cs_mapMasternodePayeeVotes, cs_mapMasternodeBlocks);
        READWRITE(obj.mapMasternodePayeeVotes, obj.mapMasternodeBlocks);
    }

private:
    // keep track of last voted height for mnw signers
    std::map<COutPoint, int> mapMasternodesLastVote GUARDED_BY(cs_mapMasternodePayeeVotes); //prevout, nBlockHeight
