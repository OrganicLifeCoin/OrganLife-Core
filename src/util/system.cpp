...
static Optional<fs::path> GetExecutablePath()
{
#ifdef WIN32
    wchar_t path_w[MAX_PATH];
    DWORD length = GetModuleFileNameW(nullptr, path_w, MAX_PATH);
    if (length == 0 || length == MAX_PATH) return Optional<fs::path>();
    return fs::path(path_w);
#elif defined(MAC_OSX)
    uint32_t size = 0;
    (void)_NSGetExecutablePath(nullptr, &size);
    if (size == 0) return Optional<fs::path>();
    std::vector<char> buf(size + 1, '\0');
    if (_NSGetExecutablePath(buf.data(), &size) != 0) return Optional<fs::path>();
    return fs::path(buf.data());
#elif defined(__linux__)
    std::vector<char> buf(4096, '\0');
    ssize_t len = readlink("/proc/self/exe", buf.data(), buf.size() - 1);
    if (len <= 0) return Optional<fs::path>();
    buf[len] = '\0';
    return fs::path(buf.data());
#else
...
static bool FindSaplingParamsInDir(const fs::path& dir, fs::path& sapling_spend, fs::path& sapling_output)
{
    const fs::path spend = dir / "sapling-spend.params";
    const fs::path output = dir / "sapling-output.params";
    if (fs::exists(spend) && fs::exists(output)) {
        sapling_spend = spend;
...
static bool CopyFileIfMissing(const fs::path& src, const fs::path& dst)
{
...
void initZKSNARKS()
{
    ...
    if (!path.empty() && fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        fParamsFound = true;
    }
    ...
    if (!fParamsFound) {
...
#ifdef MAC_OSX
        // App bundle Resources (…/.app/Contents/Resources)
        Optional<fs::path> exe = GetExecutablePath();
        if (exe) {
            const fs::path resources = exe->parent_path().parent_path() / "Resources";
            if (FindSaplingParamsInDir(resources, src_spend, src_output)) {
                LogPrintf("Found Sapling params in app bundle Resources: %s\n", resources);
            }
        }
#endif

        // Installed locations (Linux, etc.)
        if (src_spend.empty() || src_output.empty()) {
            if (FindSaplingParamsInDir(fs::path("/usr/share") / PACKAGE_TARNAME, src_spend, src_output)) {
                LogPrintf("Found Sapling params in /usr/share/%s\n", PACKAGE_TARNAME);
            } else if (FindSaplingParamsInDir(fs::path("/usr/local/share") / PACKAGE_TARNAME, src_spend, src_output)) {
                LogPrintf("Found Sapling params in /usr/local/share/%s\n", PACKAGE_TARNAME);
            }
        }

        // Source tree / build tree: walk upwards looking for ./params/
        if (src_spend.empty() || src_output.empty()) {
            Optional<fs::path> exe = GetExecutablePath();
            fs::path dir = exe ? exe->parent_path() : fs::current_path();
            for (int i = 0; i < 6; ++i) {
                if (FindSaplingParamsInDir(dir / "params", src_spend, src_output)) {
                    LogPrintf("Found Sapling params in source tree: %s\n", (dir / "params"));
                    break;
                }
        ...
        if (!src_spend.empty() && !src_output.empty()) {
            // Prefer copying into ZC_GetParamsDir when available; otherwise, initialize directly from the source paths.
            if (!path.empty()) {
                TryCreateDirectories(path);
                CopyFileIfMissing(src_spend, sapling_spend);
                CopyFileIfMissing(src_output, sapling_output);
                if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
                    fParamsFound = true;
                }
            }
            if (!fParamsFound) {
                sapling_spend = src_spend;
                sapling_output = src_output;
                fParamsFound = true;
            }
        }
    }
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    const fs::path config_path = GetConfigFile(confPath);

    if (confPath == PIVX_CONF_FILENAME && !fs::exists(config_path)) {
        try {
            TryCreateDirectories(config_path.parent_path());
            fsbridge::ofstream out(config_path);
            if (out.good()) {
                ...
                std::string rpcpassword = HexStr(Span<const uint8_t>(rand_bytes, 32));
                    ...
                    out <<
                        ...
                        "#\n"
                        "# Default to the mainnet. Set testnet=1 to use the test chain.\n"
                        "\n"
            ...
            // If we cannot create the config file (permissions, read-only FS),
            // proceed without a config file.
        }
    }
