...
namespace {
using GovVoteLockLookupMap = std::map<uint256, CTransactionRef>;
const GovVoteLockLookupMap* g_govVoteLockLookup{nullptr};

class ScopedGovVoteLockLookup
{
public:
    explicit ScopedGovVoteLockLookup(const GovVoteLockLookupMap* lookupIn) :
            prevLookup(g_govVoteLockLookup)
    {
        g_govVoteLockLookup = lookupIn;
    }

    ~ScopedGovVoteLockLookup()
    {
        g_govVoteLockLookup = prevLookup;
    }

private:
    const GovVoteLockLookupMap* prevLookup{nullptr};
};
} // namespace
...
static bool CheckService(const CService& addr, CValidationState& state)
{
    ...
    // Both IPv4 and IPv6 services are supported. Tor/I2P/CJDNS are not.
...
static bool CheckGovProposalVotingWindow(const uint256& proposalHash, int nextBlockHeight, CValidationState& state)
{
...
static bool CheckGovVoteCastTx(const CTransaction& tx,
                               ...
                               CValidationState& state)
{
    ...
    for (const auto& lockRef : pl.lockRefs) {
        ...
        if (governanceVoteIndex &&
                governanceVoteIndex->IsLockUsedForProposal(lockRef, pl.proposalHash)) {
            return state.DoS(100, false, REJECT_INVALID, "bad-govtx-lock-already-used");
        }

        CTransactionRef lockTx;
        CGovVoteLockTx lockPayload;
        bool lockPayloadLoaded = false;
        
        // Try to get lock from UTXO view first (for fresh locks in mempool)
        Coin lockCoin;
        if (view && view->GetUTXOCoin(lockRef, lockCoin)) {
            if (g_govVoteLockLookup) {
            ...
            if (!lockPayloadLoaded) {
                ...
                if (!GetTransaction(lockRef.hash, lockTx, lockHashBlock, true)) {
                    // Lock/cast relay order is not guaranteed across peers.
            ...
            if (governanceVoteIndex) {
                ...
                if (governanceVoteIndex->GetLockRecord(lockRef, lockRecord)) {
                    lockPayload.proposalHash = lockRecord.proposalHash;
                    lockPayload.lockAmount = lockRecord.lockAmount;
                    lockPayload.unlockHeight = lockRecord.unlockHeight;
                    lockPayload.ownerKeyId = lockRecord.ownerKeyId;
                    lockPayloadLoaded = true;
                    lockTx.reset(); // Mark that we don't have the full transaction
                }
            }
            
            // If still not found, try mempool transaction lookup
            if (!lockPayloadLoaded) {
                uint256 lockHashBlock;
                if (GetTransaction(lockRef.hash, lockTx, lockHashBlock, true)) {
                    lockPayloadLoaded = GetTxPayload(*lockTx, lockPayload);
        ...
        if (!lockPayloadLoaded && !lockTx) {
            // This can happen when cast arrives before lock over the network.
        ...
        if (lockPayload.proposalHash != pl.proposalHash) {
            return state.DoS(100, false, REJECT_INVALID, "bad-govtx-proposal-mismatch");
        }
        if (lockPayload.unlockHeight <= static_cast<uint32_t>(nextBlockHeight)) {
            return state.DoS(100, false, REJECT_INVALID, "bad-govtx-lock-expired");
        }
        if (!ownerSet) {
            ownerKeyId = lockPayload.ownerKeyId;
            ownerSet = true;
        } else if (ownerKeyId != lockPayload.ownerKeyId) {
