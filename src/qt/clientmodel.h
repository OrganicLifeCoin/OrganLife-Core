...
/** Model for OrganicLife network client. */
...
public:
    ...
    static bool shouldEmitNumConnectionsChanged(int previousCount, int newCount);
    ...
    void setLastNotifiedState(int height, bool initialSync)
    {
        m_last_notified_blocks.store(height, std::memory_order_relaxed);
        m_last_notified_initial_sync.store(initialSync, std::memory_order_relaxed);
    }
...
private:
    ...
    std::atomic<int> m_last_notified_blocks{-1};
    std::atomic<bool> m_last_notified_initial_sync{false};

    int numBlocksAtStartup;
    int cachedNumConnections{-1};
