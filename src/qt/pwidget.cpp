...
PWidget::PWidget(OrganicLifeGUI* _window, QWidget *parent) : QWidget((parent) ? parent : _window), window(_window) { init(); }
PWidget::PWidget(PWidget* parent) : QWidget(parent), window(parent->getWindow()) { init(); }

void PWidget::init()
{
    if (window)
        connect(window, &OrganicLifeGUI::themeChanged, this, &PWidget::onChangeTheme);
}
...
void PWidget::clearWalletModel()
{
    if (this->walletModel) {
        WalletModel* oldWalletModel = this->walletModel.data();
        QObject::disconnect(oldWalletModel, nullptr, this, nullptr);
        if (oldWalletModel->getOptionsModel()) {
            QObject::disconnect(oldWalletModel->getOptionsModel(), nullptr, this, nullptr);
        }
        if (oldWalletModel->getAddressTableModel()) {
            QObject::disconnect(oldWalletModel->getAddressTableModel(), nullptr, this, nullptr);
        }
        if (oldWalletModel->getTransactionTableModel()) {
            QObject::disconnect(oldWalletModel->getTransactionTableModel(), nullptr, this, nullptr);
        }
    }
    this->walletModel.clear();
}
