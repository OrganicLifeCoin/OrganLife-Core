...
bool TransactionRecord::TryExtractProposalName(const std::string& proposalHex, std::string& proposalNameOut)
{
    if (proposalHex.empty()) {
        return false;
    }

    const std::vector<unsigned char> vec = ParseHex(proposalHex);
    if (vec.empty()) {
        return false;
    }
    ...
    try {
        ...
        ss >> proposal;
        proposalNameOut = proposal.GetName();
        return true;
    } catch (const std::exception& e) {
        LogPrint(BCLog::QT, "Ignoring malformed proposal metadata: %s\n", e.what());
    }
    return false;
}

TransactionRecord::Type TransactionRecord::classifyCoinbaseCredit(int blockHeight, const CAmount& credit)
{
    if (credit <= 0 || !Params().GetConsensus().NetworkUpgradeActive(blockHeight, Consensus::UPGRADE_V6_0)) {
        return TransactionRecord::Generated;
    }

    const CAmount mnReward = GetMasternodePayment(blockHeight);
    if (mnReward <= 0) {
        return TransactionRecord::Generated;
    }

    // Under CTEAM's permanent legacy-MN policy, V6 coinbase credits are either
    // the fixed masternode reward or a governance payout larger than that reward.
    if (credit == mnReward) {
        return TransactionRecord::MNReward;
    }
    if (credit > mnReward) {
        return TransactionRecord::BudgetPayment;
    }
    return TransactionRecord::Generated;
}
        ...
        QList<TransactionRecord>& parts)
{
    ...
    const int nIndexMN = (int)wtx.tx->vout.size() - 1;
    CAmount mnCredit{0};
    bool hasMNReward = false;
    std::string mnAddress;
    bool isBudgetPayment = false;

    // In legacy PoS flow, the masternode reward is the last coinstake output.
    // Split it out so stake and MN rewards are represented independently in the UI/chart.
    if (nIndexMN >= 2) {
        const CTxOut& mnOut = wtx.tx->vout[nIndexMN];
        isminetype mnMine = wallet->IsMine(mnOut);
        if (mnMine) {
            const int nHeight = wtx.m_confirm.block_height;
            const CAmount expectedMNReward = GetMasternodePayment(nHeight);

            // For local coinstake decomposition, only split out the canonical MN reward output.
            // This avoids misclassifying stake-split outputs in non-enforced/edge scenarios.
            if (mnOut.nValue > expectedMNReward) {
                mnMine = ISMINE_NO;
            }
        }
        if (mnMine) {
            CTxDestination destMN;
            if (ExtractDestination(mnOut.scriptPubKey, destMN)) {
                mnAddress = EncodeDestination(destMN);
            }
            mnCredit = mnOut.nValue;
            hasMNReward = true;
        }
    }

    TransactionRecord sub(hash, wtx.GetTxTime(), wtx.tx->GetTotalSize());
    bool hasStakePart = false;

    if (isminetype mine = wallet->IsMine(wtx.tx->vout[1])) {
        // Check for cold stakes.
        if (wtx.tx->HasP2CSOutputs()) {
            sub.credit = nCredit - mnCredit;
            sub.debit = -nDebit;
            ...
            sub.credit = nCredit - nDebit - mnCredit;
            hasStakePart = true;
        }
    } else {
        //Masternode reward
        ...
        if (ExtractDestination(wtx.tx->vout[nIndexMN].scriptPubKey, destMN) && (mine = IsMine(*wallet, destMN)) ) {
            ...
            hasMNReward = true;
            mnCredit = sub.credit;
            mnAddress = sub.address;
            isBudgetPayment = sub.type == TransactionRecord::BudgetPayment;
        }
    }
    ...
    if (hasMNReward) {
        ...
        mnSub.address = mnAddress;
        mnSub.credit = mnCredit;
    ...
    for (unsigned int nOut = 0; nOut < wtx.tx->vout.size(); nOut++) {
        ...
        if (ExtractDestination(txout.scriptPubKey, address)) {
            ...
            if (sub.address.empty() && txout.scriptPubKey.StartsWithOpcode(OP_RETURN)) {
                ...
                if (TryExtractProposalName(prop, proposalName)) {
                    sub.address = "Proposal: " + proposalName;
                }
