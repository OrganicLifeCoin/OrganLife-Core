...
AskPassphraseDialog::AskPassphraseDialog(Mode mode, QWidget* parent, WalletModel* model, Context context) : ContainerDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
                                                                                                            ...
                                                                                                            btnWatch(nullptr)
{
    ...
    setCssProperty(ui->labelTitle, "text-title-dialog", true);

    ui->warningLabel->setProperty("cssClass", "text-subtitle");

    initDialogCloseButton(ui->btnEsc);
    initDraggableHeaderChrome(ui->left, ui->labelTitle, ui->btnEsc, 16);
    ...
    const int visiblePassphraseFields =
            (!ui->passEdit1->isHidden() ? 1 : 0) +
            (!ui->passEdit2->isHidden() ? 1 : 0) +
            (!ui->passEdit3->isHidden() ? 1 : 0);
    const bool compactContent = visiblePassphraseFields <= 1;

    // Let compact unlock/decrypt prompts consume less space than multi-field flows.
    ui->warningLabel->setMinimumWidth(compactContent ? 340 : 400);
    setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Minimum);
    setMinimumWidth(compactContent ? 460 : 520);
    setMaximumWidth(compactContent ? 540 : 620);
...
{
    OrganicLifeGUI* gui = static_cast<OrganicLifeGUI*>(parentWidget());
...
{
    hide();
    static_cast<OrganicLifeGUI*>(parentWidget())->showHide(true);
    openStandardDialog(
            tr("Wallet encrypted"),
            "<qt>" +
            tr("Your wallet is now encrypted. "
               "Remember that encrypting your wallet cannot fully protect "
               "your OrganicLife from being stolen by malware infecting your computer.") +
            "<br><br><b>" +
...
void AskPassphraseDialog::initWatch(QWidget *parent)
{
    ...
    btnWatch->setObjectName("btnWatchPassword");
    setCssProperty(btnWatch, "btn-watch-password", true);
    btnWatch->setChecked(false);
    const QSize watchButtonSize(28, 28);
    btnWatch->setMinimumSize(watchButtonSize);
    btnWatch->setMaximumSize(watchButtonSize);

    if (auto* layout = qobject_cast<QHBoxLayout*>(parent->layout())) {
        layout->setSpacing(std::max(layout->spacing(), 8));
        layout->addWidget(btnWatch, 0, Qt::AlignVCenter);
    }
