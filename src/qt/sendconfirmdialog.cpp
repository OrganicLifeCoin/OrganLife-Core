...
namespace {
bool IsOutgoingTxType(TransactionRecord::Type type)
{
    switch (type) {
...
QString FormatTxStatusWithBroadcastInfo(const TransactionRecord* rec)
{
    if (rec->status.status == TransactionStatus::Unconfirmed && IsOutgoingTxType(rec->type)) {
        const int peers = g_connman ? g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) : 0;
        if (peers > 0) {
            return QObject::tr("Broadcasting to %n node(s)", "", peers);
        }
        return QObject::tr("Queued for broadcast");
    }
    switch (rec->status.status) {
    case TransactionStatus::Confirmed:
        return QObject::tr("Confirmed");
    case TransactionStatus::OpenUntilDate:
        return QObject::tr("Open until date");
    case TransactionStatus::OpenUntilBlock:
        return QObject::tr("Open until block");
    case TransactionStatus::Unconfirmed:
        return QObject::tr("Unconfirmed");
    case TransactionStatus::Confirming:
        return QObject::tr("Confirming");
    case TransactionStatus::Conflicted:
        return QObject::tr("Conflicted");
    case TransactionStatus::Immature:
        return QObject::tr("Immature");
    case TransactionStatus::NotAccepted:
        return QObject::tr("Not accepted");
    }
    ...
    isConfirmDialog(_isConfirmDialog)
{
    ...
    setCssBtnSecondary(ui->btnAbandonTx);

    ui->labelWarning->setVisible(false);
    ui->gridInputs->setVisible(false);
    ui->outputsScrollArea->setVisible(false);
    ui->contentConflictActions->setVisible(false);
    ...
    connect(ui->btnAbandonTx, &QPushButton::clicked, this, &TxDetailDialog::onAbandonTransactionClicked);
}

bool TxDetailDialog::shouldShowConflictActions(TransactionStatus::Status status, bool hasWalletConflicts)
{
    switch (status) {
    case TransactionStatus::Conflicted:
    case TransactionStatus::NotAccepted:
        return true;
    case TransactionStatus::Unconfirmed:
        return hasWalletConflicts;
    default:
        return false;
    }
}

TxDetailDialog::ConflictAction TxDetailDialog::resolveConflictAction(
    TransactionStatus::Status status,
    bool hasWalletConflicts,
    bool canAbandon)
{
    if (status == TransactionStatus::Unconfirmed && hasWalletConflicts) {
        return canAbandon ? ConflictAction::Abandon : ConflictAction::HideFromHistory;
    }
    if (status == TransactionStatus::Conflicted || status == TransactionStatus::NotAccepted) {
        return ConflictAction::HideFromHistory;
    }
...
void TxDetailDialog::setData(WalletModel *_model, const QModelIndex &index)
{
    ...
    if (_tx) {
        ...
        if (rec->status.depth < 0) {
            ui->textConfirmations->setText(tr("%1 (conflict depth)").arg(rec->status.depth));
            ui->textConfirmations->setToolTip(tr("Negative confirmations mean a conflicting transaction is already confirmed in another block."));
        } else {
            ui->textConfirmations->setText(QString::number(rec->status.depth));
            ui->textConfirmations->setToolTip(QString());
        }
        ui->textDate->setText(GUIUtil::dateTimeStrWithSeconds(date));
        ui->textStatus->setText(FormatTxStatusWithBroadcastInfo(rec));
        const bool hasWalletConflicts = !_tx->GetConflicts().empty();
        conflictAction = !isConfirmDialog
            ? resolveConflictAction(rec->status.status, hasWalletConflicts, model->canAbandonTransaction(rec->hash))
            : ConflictAction::None;
        const bool showConflictActions = conflictAction != ConflictAction::None;
        ui->contentConflictActions->setVisible(showConflictActions);
        if (showConflictActions) {
            if (conflictAction == ConflictAction::Abandon) {
                ui->btnAbandonTx->setText(tr("Abandon transaction"));
                ui->btnAbandonTx->setToolTip(tr("Clear this local conflicting transaction state so it no longer blocks spending"));
            } else {
                ui->btnAbandonTx->setText(tr("Remove from wallet history"));
                ui->btnAbandonTx->setToolTip(tr("Hide this transaction from your wallet history list"));
            }
        } else {
            ui->btnAbandonTx->setToolTip(QString());
        }
...
void TxDetailDialog::onAbandonTransactionClicked()
{
    if (!model || txHash.IsNull() || conflictAction == ConflictAction::None) {
        return;
    }
    ...
    if (conflictAction == ConflictAction::HideFromHistory) {
        if (model->hideTransactionFromHistory(txHash)) {
            ui->contentConflictActions->setVisible(false);
            ui->textStatus->setText(tr("Removed from history"));
            snackBar->setText(tr("Transaction removed from wallet history"));
        } else {
            snackBar->setText(tr("Failed to remove transaction from wallet history"));
        }
        snackBar->resize(this->width(), snackBar->height());
        openDialog(snackBar, this);
        return;
    ...
    const int txDepth = model->getWalletTxDepth(txHash);
    if (txDepth < 0) {
        snackBar->setText(tr("This transaction is already conflicted in chain and cannot be abandoned."));
        snackBar->resize(this->width(), snackBar->height());
        openDialog(snackBar, this);
        return;
    }
    if (txDepth > 0) {
        snackBar->setText(tr("Confirmed transactions cannot be abandoned."));
        snackBar->resize(this->width(), snackBar->height());
        openDialog(snackBar, this);
        return;
    }

    const OperationResult res = model->abandonTransaction(txHash);

    if (res) {
        ui->contentConflictActions->setVisible(false);
        ui->textStatus->setText(tr("Abandoned"));
        snackBar->setText(tr("Transaction abandoned"));
    } else {
        snackBar->setText(QString::fromStdString(res.getError()));
    }
