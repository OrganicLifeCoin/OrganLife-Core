    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // pivx: URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(arg, &r) && !r.address.isEmpty()) {
                if (IsValidDestinationString(r.address.toStdString(), false, *CreateChainParams(CBaseChainParams::TESTNET))) {
                    SelectParams(CBaseChainParams::TESTNET);
                } else if (IsValidDestinationString(r.address.toStdString(), false, *CreateChainParams(CBaseChainParams::MAIN))) {
