...
{
    const QVariant overrideName = property("walletDisplayName");
    if (overrideName.isValid()) {
        const QString displayName = overrideName.toString().trimmed();
        if (!displayName.isEmpty()) {
            return displayName;
        }
    }

    const QString walletName = getWalletName();
    return walletName.isEmpty() ? tr("Primary") : walletName;
}

bool WalletModel::isPrimaryWallet() const
{
    const QVariant primaryOverride = property("walletPrimary");
    if (primaryOverride.isValid()) {
        return primaryOverride.toBool();
    }

    return wallet->GetName().empty();
}

QString WalletModel::hiddenTransactionsSettingsKey() const
{
    const QString walletPath = QString::fromStdString(wallet->GetPathToDBFile().string());
    const QByteArray walletPathBytes = walletPath.toUtf8();
    const QByteArray walletHash = QCryptographicHash::hash(walletPathBytes, QCryptographicHash::Sha256).toHex();
    return QStringLiteral("wallet/hiddenTransactions/%1").arg(QString::fromLatin1(walletHash));
}

void WalletModel::loadHiddenTransactions()
{
    QSettings settings;
    const QStringList hidden = settings.value(hiddenTransactionsSettingsKey()).toStringList();
    m_hiddenTransactionIds = QSet<QString>(hidden.cbegin(), hidden.cend());
}

void WalletModel::persistHiddenTransactions() const
{
    QStringList hidden = m_hiddenTransactionIds.values();
    hidden.sort();
    QSettings settings;
    settings.setValue(hiddenTransactionsSettingsKey(), hidden);
}

void WalletModel::applyHiddenTransactions()
{
    if (!transactionTableModel || m_hiddenTransactionIds.isEmpty()) {
        return;
    }
    for (const QString& txid : m_hiddenTransactionIds) {
        transactionTableModel->hideTransaction(txid);
    }
}
...
OperationResult WalletModel::createAndSendProposalFeeTx(CBudgetProposal& proposal)
{
    ...
    if (!wallet->CreateBudgetFeeTX(wtx, nHash, keyChange, BUDGET_FEE_TX_OLD)) { // 100 OLC collateral for proposal
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose)
{
    ...
    if (isStaking)
        return error("Invalid OrganicLife address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CTxDestination& address, CKeyID& keyID)
{
    if (!IsValidDestination(address))
        return error("Invalid OrganicLife address");

    const CKeyID* inKeyID = boost::get<CKeyID>(&address);
    if (!inKeyID)
        return error("Unable to get KeyID from OrganicLife address");
...
bool WalletModel::canAbandonTransaction(const uint256& txHash) const
{
    ...
    const CWalletTx& walletTx = it->second;
    return walletTx.GetDepthInMainChain() == 0 && !walletTx.InMempool() && !walletTx.isAbandoned();
}

OperationResult WalletModel::abandonTransaction(const uint256& txHash)
{
    // Match RPC behavior so results are consistent with the current chain view.
    wallet->BlockUntilSyncedToCurrentChain();

    LOCK2(cs_main, wallet->cs_wallet);
    if (!wallet->mapWallet.count(txHash)) {
        return errorOut("Invalid or non-wallet transaction id");
    }
    if (!wallet->AbandonTransaction(txHash)) {
        return errorOut("Transaction not eligible for abandonment");
    }
    return {true};
}

bool WalletModel::hideTransactionFromHistory(const uint256& txHash)
{
    if (txHash.IsNull()) {
        return false;
    }

    const QString txid = QString::fromStdString(txHash.GetHex());
    m_hiddenTransactionIds.insert(txid);
    persistHiddenTransactions();
    if (transactionTableModel) {
        transactionTableModel->hideTransaction(txid);
    }
    return true;
...
bool WalletModel::isTransactionHidden(const uint256& txHash) const
{
    if (txHash.IsNull()) {
        return false;
    }
    return m_hiddenTransactionIds.contains(QString::fromStdString(txHash.GetHex()));
}
