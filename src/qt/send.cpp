...
namespace {
void showSendBlockingError(PWidget* widget, const QString& message)
{
    widget->emitMessage(QObject::tr("Send Coins"),
                        message,
                        CClientUIInterface::MSG_ERROR | CClientUIInterface::MODAL,
                        nullptr);
}

LoadingDialog::Content makePrepareLoadingContent(const QList<SendCoinsRecipient>& recipients,
                                                 int displayUnit,
                                                 bool isShieldedTx)
{
    LoadingDialog::Content content;
    content.eyebrow = QObject::tr("Processing");
    content.title = QObject::tr("Preparing transaction");
    content.supportText = QObject::tr("Selecting inputs, calculating fees, and validating outputs.");
    ...
    content.items.append({QObject::tr("Recipients"), QString::number(recipients.size())});
    content.items.append({QObject::tr("Total"), BitcoinUnits::formatWithUnit(displayUnit, totalAmount)});
    content.items.append({QObject::tr("Route"), isShieldedTx ? QObject::tr("Shielded") : QObject::tr("Transparent")});
    return content;
}

LoadingDialog::Content makeBroadcastLoadingContent(int peersCount)
{
    LoadingDialog::Content content;
    content.eyebrow = QObject::tr("Network");
    content.title = QObject::tr("Broadcasting transaction");
    content.supportText = peersCount > 0
            ? QObject::tr("Submitting the transaction to connected peers.")
            : QObject::tr("Submitting the transaction to the network.");

    if (peersCount > 0) {
        content.items.append({QObject::tr("Peers"), QObject::tr("%n node(s)", "", peersCount)});
    }
    return content;
}
}

bool SendWidget::shouldRequirePeerConnections(bool hasClientModel, int peersCount)
{
    return hasClientModel && peersCount <= 0;
}

bool SendWidget::shouldAutoAbandonFailedCommit(const WalletModel::SendCoinsReturn& sendStatus)
{
    return sendStatus.status == WalletModel::TransactionCommitFailed &&
           sendStatus.commitRes.status == CWallet::CommitStatus::NotAccepted &&
    ...
    ui(new Ui::send)
{
    ...
    setCssProperty(ui->btnChangeAddress, "screen-side-option", true);
    ...
    connect(ui->checkBoxDelegations, &QCheckBox::checkStateChanged, this, [this](Qt::CheckState) { onCheckBoxChanged(); });
#else
    connect(ui->checkBoxDelegations, &QCheckBox::stateChanged, this, [this](int) { onCheckBoxChanged(); });
#endif
...
void SendWidget::clearWalletModel()
{
    cleanupNewPQKeys();
...
void SendWidget::onSendClicked()
{
    if (!walletModel || !walletModel->getOptionsModel()) {
        showSendBlockingError(this, tr("Wallet is not ready yet. Please wait a few seconds and try again."));
        return;
    }
    ...
    for (SendMultiRow* entry : entries) {
        ...
        if (entry && entry->validate()) {
            ...
            showSendBlockingError(this, tr("Invalid entry. Please check recipient address and amount."));
            return;
        }
    }

    if (recipients.isEmpty()) {
        showSendBlockingError(this, tr("No recipients set."));
        return;
    }
                             ...
                             const std::function<bool(QList<SendCoinsRecipient>&)>& func)
{
    ...
    auto ptrUnlockedContext = std::make_unique<WalletModel::UnlockContext>(operationWallet->requestUnlock());
    if (operationWallet.isNull() || walletModel != operationWallet) return;
    if (!ptrUnlockedContext->isValid()) {
        // Unlock wallet was cancelled
        showSendBlockingError(this, tr("Cannot send, wallet locked."));
        return;
    }
    ...
    if (isProcessing || ptrModelTx) {
        showSendBlockingError(this, tr("Another transaction is still being processed. Please wait and try again."));
        return;
    }
    ...
    if (Params().SupportsPQ()) {
        ...
        LoadingDialog *dialog = new LoadingDialog(window, makePrepareLoadingContent(recipients, nDisplayUnit, isShieldedTx));
    ...
    } else if (processingResultError && !processingResultError->isEmpty()) {
        showSendBlockingError(this, *processingResultError);
    }
    cleanupNewPQKeys();
...
bool SendWidget::sendFinalStep()
{
    ...
    if (dialog->isConfirm()) {
        if (clientModel) {
            if (!clientModel->getNetworkActive()) {
                clientModel->setNetworkActive(true);
            }
            const int peerCount = clientModel->getNumConnections(CONNECTIONS_ALL);
            if (shouldRequirePeerConnections(true, peerCount)) {
                showSendBlockingError(this, tr("No active peer connections. Wait for the wallet to reconnect before sending."));
                dialog->deleteLater();
                return false;
            }
        }
    if (Params().SupportsPQ()) {
            ...
            pqPreparationTracked = false;
            pqPreparationWallet.clear();
    ...
    if (Params().SupportsPQ()) {
            ...
            const int peersCount = clientModel ? clientModel->getNumConnections(CONNECTIONS_ALL) : 0;
            LoadingDialog* broadcastDialog = new LoadingDialog(window, makeBroadcastLoadingContent(peersCount));
            broadcastDialog->execute(this, REQUEST_BROADCAST_TX);
            openDialogWithOpaqueBackgroundFullScreen(broadcastDialog, window);
        }

        // process send status and on error generate message shown to user
        ...
        if (shouldAutoAbandonFailedCommit(sendCoinsStatus)) {
            const OperationResult abandonRes = walletModel->abandonTransaction(sendCoinsStatus.commitRes.hashTx);
            if (abandonRes) {
                inform(tr("Send failed and the pending transaction was cleared automatically. You can retry now."));
            } else {
                showSendBlockingError(this, tr("Send failed and the transaction could not be cleared automatically. "
                                               "Open transaction details and use Abandon transaction."));
            }
        }

        if (sendCoinsStatus.status == WalletModel::OK) {
            clearAll(false);
            inform(tr("Transaction queued for broadcast"));
            dialog->deleteLater();
            return true;
        }
    }
...
void SendWidget::run(int type)
{
    ...
    } else if (type == REQUEST_BROADCAST_TX) {
        if (!isProcessing) {
            isProcessing = true;
            if (!walletModel || !ptrModelTx) {
                sendCoinsStatus = WalletModel::SendCoinsReturn(WalletModel::TransactionCommitFailed);
            } else {
                sendCoinsStatus = walletModel->sendCoins(*ptrModelTx);
            }
...
void SendWidget::beginPQPreparation()
{
    pqPreparationWallet = walletModel;
    pqAddressesBeforePrepare = walletModel->getWallet()->GetPQAddresses();
    pqPreparationTracked = true;
}

bool SendWidget::cleanupNewPQKeys()
{
    if (!pqPreparationTracked) return true;

    WalletModel* trackedWallet = pqPreparationWallet.data();
    bool cleaned = trackedWallet != nullptr;
    if (trackedWallet) {
        for (const auto& address : trackedWallet->getWallet()->GetPQAddresses()) {
            if (std::find(pqAddressesBeforePrepare.begin(), pqAddressesBeforePrepare.end(), address) ==
                    pqAddressesBeforePrepare.end()) {
                cleaned &= trackedWallet->getWallet()->ErasePQAddress(address);
            }
        }
    }
    pqAddressesBeforePrepare.clear();
    ...
    pqPreparationWallet.clear();
    if (!cleaned) {
        emitMessage(tr("Send Coins"),
                    tr("An unused PQ change key could not be removed. Back up this wallet before closing it."),
