...
AddressesWidget::AddressesWidget(OrganicLifeGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    setCssProperty(ui->btnAddContact, "screen-side-option", true);
    ...
    // Add Contact
    ui->layoutNewContact->setAttribute(Qt::WA_StyledBackground, true);
    setCssProperty(ui->layoutNewContact, "screen-side-card");
    ui->verticalLayout_71->setContentsMargins(16, 16, 16, 16);
    ui->verticalLayout11->setSpacing(12);
    ui->verticalLayout_2->setSpacing(12);

    // Name
    setCssProperty(ui->labelName, "screen-side-section-title");
    setCssEditLine(ui->lineEditName, true);

    // Address
    setCssProperty(ui->labelAddress, "screen-side-section-title");
    setCssEditLine(ui->lineEditAddress, true);
    ui->lineEditAddress->setValidator(new QRegularExpressionValidator(QRegularExpression("^[A-Za-z0-9]*$"), ui->lineEditAddress));
