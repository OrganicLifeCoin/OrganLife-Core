...
void RPCConsole::setClientModel(ClientModel* model)
{
    ...
    if (model && clientModel->getPeerTableModel() && clientModel->getBanTableModel()) {
        ...
        setMasternodeCount(model->getMasternodesCountString());
...
void RPCConsole::setWalletModel(WalletModel* model)
{
    ...
    if (auto* window = qobject_cast<PIVXGUI*>(parentWidget())) {
        if (WalletModel* primaryWalletModel = window->getWallet(window->primaryWalletName())) {
            ...
            walletModel = implicitPrimaryWalletModel;
        }
    }
#endif
    if (walletModel) {
        ui->wallet_path->setText(walletModel->getWalletPath());
        Q_EMIT walletNameChanged(walletModel->getWalletName(), true);
    } else {
...
void RPCConsole::clear()
{
    ...
    message(RPCExecutor::CMD_REPLY, (tr("Welcome to the 1776$ RPC console.") + "<br>" +
...
{
    if (strMasternodes.isEmpty()) {
        ui->masternodeCount->setText(tr("Loading..."));
    } else {
...
void RPCConsole::startExecutor()
{
    ...
    connect(this, &RPCConsole::walletNameChanged, executor, &RPCExecutor::setWalletName);
