...
int64_t GetCycleSeconds(const GovernanceModel* govModel)
{
    const int64_t targetSpacing = Params().GetConsensus().nTargetSpacing > 0 ? Params().GetConsensus().nTargetSpacing : 1;
    const int cycleBlocks = (govModel && govModel->getNumBlocksPerBudgetCycle() > 0) ?
            govModel->getNumBlocksPerBudgetCycle() : Params().GetConsensus().nBudgetCycleBlocks;
    return static_cast<int64_t>(cycleBlocks > 0 ? cycleBlocks : 1) * targetSpacing;
}

QString FormatCycleCount(int cycles)
{
    return cycles == 1 ? QObject::tr("1 cycle") : QObject::tr("%1 cycles").arg(cycles);
}
} // namespace
    ...
    icConfirm3(new QPushButton(this))
{
    ...
    constexpr int kCreateProposalMinWidth = 760;
    constexpr int kCreateProposalMinHeight = 620;
    setMinimumSize(kCreateProposalMinWidth, kCreateProposalMinHeight);
    resize(std::max(width(), kCreateProposalMinWidth), std::max(height(), kCreateProposalMinHeight));
    ...
    initDialogCloseButton(ui->pushButtonSkip);
    auto* headerTitle = new QLabel(tr("Create Proposal"), this);
    headerTitle->setAlignment(Qt::AlignCenter);
    setCssProperty(headerTitle, "text-title-dialog");
    initDraggableHeaderChrome(ui->frame, headerTitle, ui->pushButtonSkip, 16);
...
void CreateProposalDialog::setupPageOne()
{
    ...
    setEditBoxStyle(ui->labelURL, ui->lineEditURL, "e.g https://freedom.buzz/proposals/best_proposal_ever");
...
void CreateProposalDialog::setupPageTwo()
{
    ...
    if (govModel) {
        ui->lineEditMonths->setMaximum(govModel->getPropMaxPaymentsCount());
    }
...
void CreateProposalDialog::setupPageThree()
{
    ...
    setCssProperty({ui->labelResultNameTitle,
                   ...
                   "text-title");
    ui->labelResultMonthsTitle->setText(tr("Cycles"));
...
void CreateProposalDialog::loadSummary()
{
    ...
    const int cycles = ui->lineEditMonths->value();
    const int64_t totalSeconds = static_cast<int64_t>(cycles) * GetCycleSeconds(govModel);
    ui->labelResultMonths->setText(tr("%1 (~%2)").arg(FormatCycleCount(cycles), FormatDurationLabel(totalSeconds)));
...
{
    int cycles = ui->lineEditMonths->value();
