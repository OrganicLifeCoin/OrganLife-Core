...
{
    return QLocale().toString(date.date(), QLocale::ShortFormat) + QString(" ") + date.toString("hh:mm:ss");
}

QString dateTimeStr(qint64 nTime)
{
    return dateTimeStr(dateTimeFromTimeT(nTime));
}

QDateTime dateTimeFromTimeT(qint64 nTime)
{
#if (QT_VERSION >= QT_VERSION_CHECK(5, 8, 0))
    return QDateTime::fromSecsSinceEpoch(nTime);
#else
    return QDateTime::fromTime_t(static_cast<uint>(nTime));
#endif
}
...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter 1776$ address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
{
    // return if URI is not valid or does not use a recognized scheme
    if (!uri.isValid())
        return false;
    const QString scheme = uri.scheme();
    if (scheme != QString(URI_SCHEME) && scheme != QString(URI_SCHEME_FALLBACK))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    ...
    } else if (uri.startsWith(URI_SCHEME ":", Qt::CaseInsensitive)) {
        uri.replace(0, std::strlen(URI_SCHEME) + 1, URI_SCHEME_FALLBACK ":");
    } else if (uri.startsWith(URI_SCHEME_FALLBACK "://", Qt::CaseInsensitive)) {
        uri.replace(0, std::strlen(URI_SCHEME_FALLBACK) + 3, URI_SCHEME_FALLBACK ":");
    }
...
QString getSaveFileName(QWidget* parent, const QString& caption, const QString& dir, const QString& filter, QString* selectedSuffixOut)
{
    ...
    {
        static const QRegularExpression filter_re(QStringLiteral(".* \\(\\*\\.(.*)[ \\)]"));
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart) {
        ...
        if (SUCCEEDED(hres)) {
            ...
            QString strArgs = "-min";
            // Set -testnet option
            strArgs += QString::fromStdString(strprintf(" -testnet=%d", gArgs.GetBoolArg("-testnet", false)));
...
bool isExternal(QString theme)
{
    ...
    return theme != QLatin1String("default") && theme != QLatin1String("default-dark");
}
