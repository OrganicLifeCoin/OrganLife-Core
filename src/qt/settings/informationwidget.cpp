...
SettingsInformationWidget::SettingsInformationWidget(OrganicLifeGUI* _window,QWidget *parent) :
...
{
    if (!rpcConsole) {
        rpcConsole = new RPCConsole(getWindow() ? static_cast<QWidget*>(getWindow()) : this);
...
void SettingsInformationWidget::run(int type)
{
    if (type == REQUEST_UPDATE_COUNTS) {
        ...
        QMetaObject::invokeMethod(this, "setNumBlocks",
                                  Qt::QueuedConnection, Q_ARG(int, clientModel ? clientModel->getLastBlockProcessedHeight() : 0));
    }
}
