    ...
    ui(new Ui::SettingsFaqWidget)
{
    ...
    QString introContent = formatFAQContent(
        formatFAQParagraph(
            tr("%1 mainnet has a fixed supply cap of 177,600,000 coins, "
               "a 2 minute target block time, and optional Sapling shielded "
               "transactions for privacy.").arg(PACKAGE_NAME)) +
        formatFAQParagraph(
            tr("%1 starts with a short Proof of Work bootstrap and then transitions "
               "to Proof of Stake. After PoS activates, wallet owners can stake and "
               "masternodes with %2 collateral can participate in governance and "
               "earn rewards.")
                .arg(PACKAGE_NAME)
    ...
    QString unspendablePIVContent = formatFAQContent(
        formatFAQParagraph(
            tr("Newly received %1 requires 6 confirmations on the network "
               "to become eligible for spending which can take ~6 minutes.").arg(PACKAGE_NAME)) +
        formatFAQParagraph(
            tr("Your %1 wallet also needs to be completely synchronized "
               "to see and spend balances on the network.").arg(PACKAGE_NAME)));
    ...
    QString stakeContent = formatFAQContent(
        formatFAQOrderedList(
            ...
            formatFAQListItem(tr("You must have a balance of %1 with a minimum of 600 confirmations.").arg(PACKAGE_NAME)) +
    ...
    QString supportContent = formatFAQContent(
        formatFAQParagraph(
            tr("We have support channels in our official chat groups.")));
    ...
    QString masternodeContent = formatFAQContent(
        ...
        formatFAQParagraph(
            tr("Masternode Perks:") +
            formatFAQUnorderedList(
                formatFAQListItem(tr("Participate in %1 Governance").arg(PACKAGE_NAME)) +
                formatFAQListItem(tr("Earn Masternode Rewards")) +
                formatFAQListItem(tr("Commodity option for future sale")) +
                formatFAQListItem(tr("Help secure the %1 network").arg(PACKAGE_NAME)))) +
        formatFAQParagraph(
            tr("Requirements:") +
            formatFAQUnorderedList(
