...
/** Interface to OrganicLife wallet from Qt view code. */
...
public:
    ...
    bool canAbandonTransaction(const uint256& txHash) const;
    OperationResult abandonTransaction(const uint256& txHash);
    bool hideTransactionFromHistory(const uint256& txHash);
    bool isTransactionHidden(const uint256& txHash) const;
...
private:
    ...
    QString hiddenTransactionsSettingsKey() const;
    void loadHiddenTransactions();
    void persistHiddenTransactions() const;
    void applyHiddenTransactions();
