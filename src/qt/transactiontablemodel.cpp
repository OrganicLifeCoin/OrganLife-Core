...
static bool IsOutgoingTxType(TransactionRecord::Type type)
{
    switch (type) {
...
namespace {
bool IsGovernanceVoteInternalLabel(const QString& label)
{
    return label == QLatin1String("gov-vote-lock") ||
           label == QLatin1String("gov-vote-cast") ||
           label == QLatin1String("gov-vote");
}

QString FormatInternalTxLabel(const QString& label)
{
    if (label == QLatin1String("gov-vote-lock")) {
        return QCoreApplication::translate("TransactionTableModel", "Vote lock for proposal");
    }
    if (label == QLatin1String("gov-vote-cast")) {
        return QCoreApplication::translate("TransactionTableModel", "Vote cast for proposal");
    }
    if (label == QLatin1String("gov-vote")) {
        return QCoreApplication::translate("TransactionTableModel", "Proposal vote");
    }
    return label;
}
} // namespace
...
void TransactionTableModel::hideTransaction(const QString& hash)
{
    updateTransaction(hash, CT_UPDATED, false);
}
...
void TransactionTableModel::updateTransaction(const QString& hash, int status, bool showTransaction)
{
    ...
    bool effectiveShowTransaction = showTransaction;
    if (walletModel && walletModel->isTransactionHidden(updated)) {
        effectiveShowTransaction = false;
    }

    TransactionRecord rec(0);
    priv->updateWallet(updated, status, effectiveShowTransaction, rec);

    if (effectiveShowTransaction && !rec.isNull())
        Q_EMIT txArrived(hash, rec.isCoinStake(), rec.isMNReward(), rec.isAnyColdStakingType());
}

void TransactionTableModel::reloadWalletData()
{
    beginResetModel();
    priv->refreshWallet();
    endResetModel();
...
QString TransactionTableModel::formatTxStatus(const TransactionRecord* wtx) const
{
    ...
    case TransactionStatus::Unconfirmed:
        if (IsOutgoingTxType(wtx->type)) {
            const int peers = g_connman ? g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) : 0;
            status = peers > 0 ? tr("Broadcasting to %n node(s)", "", peers) : tr("Queued for broadcast");
        } else {
...
QString TransactionTableModel::lookupAddress(const std::string& address, bool tooltip) const
{
    ...
    label = FormatInternalTxLabel(label);
    QString description;
    if (!label.isEmpty()) {
...
QString TransactionTableModel::formatTxType(const TransactionRecord* wtx) const
{
    ...
    case TransactionRecord::SendToOther:
        return tr("Sent to");
    case TransactionRecord::SendToSelf: {
        const QString label = walletModel->getAddressTableModel()->labelForAddress(QString::fromStdString(wtx->address));
        if (IsGovernanceVoteInternalLabel(label)) {
            return tr("Proposal vote (self)");
        }
...
QVariant TransactionTableModel::data(const QModelIndex& index, int role) const
{
    ...
    case SizeRole:
        return rec->size;
    case DateRole:
        return GUIUtil::dateTimeFromTimeT(static_cast<qint64>(rec->time));
    case WatchonlyRole:
    ...
    case LabelRole:
        return FormatInternalTxLabel(walletModel->getAddressTableModel()->labelForAddress(QString::fromStdString(rec->address)));
    case AmountRole:
        return qint64(rec->credit + rec->debit);
...
static void ShowProgress(TransactionTableModel* ttm, const std::string& title, int nProgress)
{
    ...
    if (nProgress == 100) {
        ...
        if (ShouldReloadTransactionModel(vQueueNotifications.size())) {
            QMetaObject::invokeMethod(ttm, "reloadWalletData", Qt::QueuedConnection);
        } else {
