...
bool ClientModel::shouldEmitNumConnectionsChanged(int previousCount, int newCount)
{
...
QDateTime ClientModel::getLastBlockDate() const
{
    ...
    return GUIUtil::dateTimeFromTimeT(static_cast<qint64>(nTime));
}
...
void ClientModel::updateTimer()
{
    ...
    const int currentConnections = getNumConnections();
    if (shouldEmitNumConnectionsChanged(cachedNumConnections, currentConnections)) {
        cachedNumConnections = currentConnections;
        Q_EMIT numConnectionsChanged(currentConnections);
    }

    Q_EMIT bytesChanged(getTotalBytesRecv(), getTotalBytesSent());

    // Best-effort UI refresh for short/fast initial syncs: NotifyBlockTip can be
    // throttled during initial sync (MODEL_UPDATE_DELAY), which may cause the UI
    // to miss the last tip update when only a few blocks are processed quickly.
    const int curBlocks = getNumBlocks();
    const bool curInitialSync = cachedInitialSync.load(std::memory_order_relaxed);
    const int lastBlocks = m_last_notified_blocks.load(std::memory_order_relaxed);
    const bool lastInitialSync = m_last_notified_initial_sync.load(std::memory_order_relaxed);
    if (curBlocks != lastBlocks || curInitialSync != lastInitialSync) {
        setLastNotifiedState(curBlocks, curInitialSync);
        Q_EMIT numBlocksChanged(curBlocks);
    }
}
...
{
    if (!shouldEmitNumConnectionsChanged(cachedNumConnections, numConnections)) {
        return;
    }
    cachedNumConnections = numConnections;
...
{
    return GUIUtil::dateTimeFromTimeT(static_cast<qint64>(nClientStartupTime)).toString();
}
...
{
    // Always keep cached state up to date, even if UI notifications are throttled.
    ...
    if (!initialSync || now - nLastBlockTipUpdateNotification > MODEL_UPDATE_DELAY) {
        ...
        clientmodel->setLastNotifiedState(pIndex->nHeight, initialSync);
