...
enum class ConnmanThreadStallEvent
{
    NONE = 0,
    STALLED,
    RECOVERED
...
private:
    ...
    {
        ThreadHeartbeatState(const char* thread_name, int64_t stall_timeout)
            : stall_timeout_us(stall_timeout), name(thread_name)
        {}

        std::atomic<bool> active{false};
        std::atomic<int64_t> last_heartbeat_us{0};
        std::atomic<bool> stalled_warning_logged{false};
    ...
    void ThreadStallMonitor();
    void SetThreadHeartbeatActive(ThreadHeartbeatState& state, bool active);
    void MarkThreadHeartbeat(ThreadHeartbeatState& state);
    void CheckThreadStall(ThreadHeartbeatState& state, int64_t now_us);
    void MaybeAutoRecoverNetThread(int64_t now_us);
    void MaybeReenableNetworkAfterRecovery(int64_t now_us);
    void JoinThreadWithProgress(std::thread& thread, const char* name, ThreadHeartbeatState* heartbeat = nullptr);
    ...
    std::atomic<bool> fDumpBanlistPending{false};
    ...
    bool fMsgProcWake GUARDED_BY(mutexMsgProc){false};

    std::condition_variable_any condMsgProc;
    Mutex mutexMsgProc;
    ...
    std::thread threadStallMonitor;

    ThreadHeartbeatState m_threadHeartbeatNet{"net", 60 * 1000000};
    ThreadHeartbeatState m_threadHeartbeatOpenCon{"opencon", 60 * 1000000};
    ThreadHeartbeatState m_threadHeartbeatAddCon{"addcon", 120 * 1000000};
    ThreadHeartbeatState m_threadHeartbeatMsgProc{"msghand", 60 * 1000000};
    std::atomic<bool> m_netStallAutoRecoverEnabled{false};
    std::atomic<int64_t> m_netStallLastRecoveryUs{0};
    std::atomic<int64_t> m_netStallNetworkReenableUs{0};
...
extern RecursiveMutex cs_mapAlreadyAskedFor;
...
public:
    ...
    std::multimap<int64_t, CInv> mapAskFor GUARDED_BY(cs_inventory);
    std::set<uint256> setAskFor GUARDED_BY(cs_inventory);
