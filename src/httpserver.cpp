...
CService HTTPRequest::GetPeer()
{
    ...
    if (con) {
        ...
        const char* address = "";
        uint16_t port = 0;
#if defined(EVENT__NUMERIC_VERSION) && EVENT__NUMERIC_VERSION >= 0x02020001
        evhttp_connection_get_peer(con, &address, &port);
#else
        char* address_out = nullptr;
        evhttp_connection_get_peer(con, &address_out, &port);
        address = address_out ? address_out : "";
#endif
